...
export interface PackRevIndex {
  ...
  /** The embedded copy of the pack checksum. Retained, never compared (Pin H R10b). */
 ...
 * tautological and an appended-bytes corruption undetectable (Pin B).
 */
export function parsePackRevIndex(
  ...
  // No comparison against `digestLength` here — Pin H R16 pins that git
  // accepts a `hashId` that disagrees with the repository's own hash width.
 ...
 * not a parse refusal, because git compares it like any other value (§D11).
 */
