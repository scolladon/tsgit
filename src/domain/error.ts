 ...
 * line, or exceeds the parser's entry-count cap). Carries a `reason` and the
 * 1-based `lineNumber` of the offending line — never the raw line bytes,
 * since `.git/shallow` content is remote-influenced (a fetch persists
 * whatever `shallow` pkt-lines the server sends).
 */
...
function extractDetail(data: TsgitErrorData): string {
  switch (data.code) {
    ...
    case 'GITATTRIBUTES_FILE_TOO_LARGE':
      return `.gitattributes too large: ${basename(data.path)} size=${data.size} limit=${data.limit}`;
    ...
    case 'SMUDGE_FILTER_FAILED':
      return `smudge filter '${data.filter}' failed for ${basename(data.path)} (exit ${data.exitCode})`;
    ...
    case 'CONFIG_INVALID_ENUM_VALUE':
      return `invalid value for '${data.key}': '${data.value}' in file ${data.source}`;
