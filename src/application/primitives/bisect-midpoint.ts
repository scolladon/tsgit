...
import { readCommitMeta } from './internal/read-commit-meta.js';
...
const readCommitEntry = async (ctx: Context, id: ObjectId): Promise<CommitEntry> => {
  ...
  if (meta === undefined) throw invalidWalkInput(`bisectMidpoint: ${id} is not a commit`);
  return { date: meta.committerDate, parents: meta.parents };
};
