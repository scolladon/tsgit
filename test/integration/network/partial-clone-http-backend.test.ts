...
describe.skipIf(SKIP_REASON !== false)(
  ...
  () => {
    ...
    describe('Given a blob:none clone, When config is inspected', () => {
      it('Then the promisor block and .promisor sentinel exist', async () => {
        // Arrange
    ...
    describe('Given a blob omitted by the filter, When readBlob is called', () => {
      it('Then it is lazy-fetched from the promisor', async () => {
        // Arrange
        const workDir = await mkdtemp(path.join(os.tmpdir(), 'tsgit-pc-'));
        workDirs.push(workDir);
        const repo = await cloneInto(workDir);
    ...
    describe('Given several omitted blobs, When fetchMissing is called', () => {
      it('Then they are pulled in one batch and git can read the repo', async () => {
        // Arrange
        const workDir = await mkdtemp(path.join(os.tmpdir(), 'tsgit-pc-'));
        workDirs.push(workDir);
        const repo = await cloneInto(workDir);
        ...
        expect(log.split('\n').filter((line) => line.length === 40).length).toBeGreaterThanOrEqual(
          1,
        );
