...
describe.skipIf(!GIT_AVAILABLE)('head symlink interop', () => {
  ...
  describe('Given HEAD is a symlink whose link text is not a valid refname', () => {
    ...
    describe('When the link text names a file holding another branch’s oid and HEAD is resolved then committed through', () => {
      it('Then it resolves detached to that oid, symbolic-ref refuses, and commit writes a regular HEAD file leaving the target unchanged', async () => {
        // Arrange
        const { gitDir, tsgitDir, ctx } = await setupTwins('oid-file', 'refs/heads/a..b', (dir) =>
    ...
    describe('When the link text names a file holding a symbolic-ref line and HEAD is resolved then committed through', () => {
      it('Then it resolves symbolic to the named branch on both, and commit advances that branch leaving the link in place', async () => {
        // Arrange
        const { gitDir, tsgitDir, ctx } = await setupTwins(
          'symref-line',
          'refs/heads/a..b',
          (dir) => writeLooseRef(dir, 'a..b', 'ref: refs/heads/side\n'),
        );
    ...
    describe('When the link text is a `..`-relative path resolving to a valid refname and HEAD is resolved then committed through', () => {
      it('Then it resolves detached to that branch and commit writes a regular HEAD file leaving the branch unchanged', async () => {
        // Arrange
        const { gitDir, tsgitDir, ctx } = await setupTwins(
          'relative-path',
          'refs/heads/../heads/side',
        );
    ...
    describe.each([
      ...
      'When $label names a file holding the main branch’s oid and HEAD is resolved then committed through',
      ({ refName, linkText }) => {
    ...
    describe('When the link text names a branch that does not exist yet and HEAD is resolved then committed through', () => {
      it('Then it resolves symbolic on both and commit creates the branch, keeping the link', async () => {
        // Arrange
        const { gitDir, tsgitDir, ctx } = await setupTwins(
          'dangling-branch',
          'refs/heads/valid-dangling',
        );
        ...
        expect(isHeadSymlink(gitDir)).toBe(true);
        expect(isHeadSymlink(tsgitDir)).toBe(true);
        expect(git(gitDir, 'rev-parse', 'refs/heads/valid-dangling').trim()).toMatch(/^[0-9a-f]+$/);
    ...
    describe('When the link text names an absent target and HEAD is resolved, status runs, and commit is attempted — a residual (git and tsgit disagree)', () => {
      it('Then git reads a detached, unborn HEAD and writes it on commit, while tsgit refuses REF_NOT_FOUND throughout', async () => {
        // Arrange
        const { gitDir, tsgitDir, ctx } = await setupTwins('absent-target', 'refs/heads/a..b');

        // Act
        const gitRevParse = tryRunGitWithExit(['-C', gitDir, 'rev-parse', 'HEAD']);
        const gitStatus = tryRunGitWithExit(['-C', gitDir, 'status', '--porcelain=v2', '--branch']);
    ...
    describe('When the link text names a directory and HEAD is resolved, status runs, and commit is attempted — a residual (git and tsgit disagree on status)', () => {
      it('Then git reads a detached, unborn HEAD via status while tsgit refuses, and both refuse commit', async () => {
        // Arrange
        const { gitDir, tsgitDir, ctx } = await setupTwins(
          'directory-target',
          'refs/heads/a..b',
          (dir) => mkdirSync(path.join(dir, '.git', 'refs', 'heads', 'a..b'), { recursive: true }),
        );
