/**
 * Calendar-component extraction for the default git-date formatter.
 * `wallClockParts` shifts the UTC instant by the identity's stored offset and
 * reads UTC getters, so the components are the time in the identity's own zone
 * (host-independent). The struct carries indices; callers name them through
