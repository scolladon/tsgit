...
describe('integration — sparse reset/merge (memory adapter)', () => {
  describe('Given a sparse cone repo, When reset --mixed rebuilds the index', () => {
    it('Then the excluded path stays skip-worktree and status is clean', async () => {
  ...
  describe('Given a sparse cone repo with a dirty in-cone file, When reset --hard runs', () => {
    it('Then the file reverts, excluded files stay absent, and status is clean', async () => {
      // Arrange
      const ctx = createMemoryContext();
      await init(ctx);
      await ctx.fs.writeUtf8(`${ctx.layout.workDir}/src/app/main.ts`, 'export const main = 1;\n');
  ...
  describe('Given a sparse cone repo, When a conflicting merge runs', () => {
    it('Then the excluded clean file stays absent and skip-worktree', async () => {
...
describe.skipIf(GIT === undefined)('integration — sparse reset interop with canonical git', () => {
  ...
  describe('Given a sparse repo where tsgit ran reset --mixed, When git reads the index', () => {
    it('Then the excluded path keeps its skip-worktree flag and git status is clean', async () => {
