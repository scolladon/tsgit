...
describe('integration — sparse checkout (memory adapter)', () => {
  describe('Given a multi-directory repo, When set/reapply/disable cone', () => {
    it('Then disk, index flags, status, and commit tree stay git-faithful', async () => {
  ...
  describe('Given a repo, When set non-cone gitignore-style patterns', () => {
    it('Then materialization and list match the raw patterns', async () => {
      // Arrange.
      const tree: Readonly<Record<string, string>> = {
        'README.md': '# repo\n',
  ...
  describe('Given a sparse cone repo, When checkout switches branch', () => {
    it('Then only in-cone files of the new tree materialise', async () => {
      ...
      await sut.fs.writeUtf8(
        `${sut.layout.workDir}/src/lib/helper.ts`,
  ...
  describe('Given a sparse repo, When add --all runs', () => {
    it('Then skip-worktree entries are not phantom-removed; after disable add --all is normal', async () => {
  ...
  describe('Given a modified out-of-cone file, When set narrows the cone', () => {
    it('Then it is retained; force removes it', async () => {
  ...
  describe('Given a sparse cone repo, When add folds in another directory', () => {
    it('Then list reports both directories', async () => {
      // Arrange.
      const tree: Readonly<Record<string, string>> = {
        'src/app/main.ts': 'export const main = 1;\n',
        'src/lib/core.ts': 'export const core = 2;\n',
...
describe.skipIf(GIT === undefined)(
  ...
  () => {
    ...
    describe('Given an index tsgit wrote with a skip-worktree entry, When git ls-files -t reads it', () => {
      it('Then the S flag is shown', async () => {
    ...
    describe('Given a sparse-checkout file tsgit wrote, When git sparse-checkout list/reapply runs', () => {
      it('Then git accepts it', async () => {
