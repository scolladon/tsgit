...
describe('integration/submodules', () => {
  describe('Given a real repo with one gitlink + .gitmodules, When repo.submodule.list()', () => {
    it('Then surfaces the entry', async () => {
      // Arrange
      ...
      try {
        ...
        // Act
        const result = await repo.submodule.list();
  ...
  describe('Given a real repo with an absorbed nested submodule, When repo.submodule.list({ recursive: true })', () => {
    it('Then descends into the nested store', async () => {
      // Arrange
      const repo = await openRepository({ cwd: tmpDir });
      try {
        await repo.init();
        ...
        const { writeObject, writeTree } = await import(
        ...
        // Act
        const result = await repo.submodule.list({ recursive: true });
        await childRepo.dispose();

        // Assert
        expect(result.entries.map((e) => ({ depth: e.depth, path: e.path }))).toEqual([
          { depth: 0, path: 'vendorfoo' },
          { depth: 1, path: 'vendorfoo/inner' },
        ]);
        expect(result.entries[1]?.parent).toBe('vendorfoo');
      } finally {
        await repo.dispose();
      }
    });
