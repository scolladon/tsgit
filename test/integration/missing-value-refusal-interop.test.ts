...
describe.skipIf(!GIT_AVAILABLE)('missing-value-refusal interop', () => {
  ...
  /**
   * A fixture controlling line numbers: a valued `url` (line 4) with a valueless
   * `pushurl` (line 5). git dies on the valueless `pushurl` even though a usable
   * url is present, on BOTH push and fetch.
   * Line 1: [core]
   * Line 2: \trepositoryformatversion = 0
   * Line 3: [remote "origin"]
   * Line 4: \turl = /tmp/nonexistent
   * Line 5: \tpushurl          <- valueless
   */
  const VALUELESS_PUSHURL_VALUED_URL_FIXTURE =
    '[core]\n\trepositoryformatversion = 0\n[remote "origin"]\n\turl = /tmp/nonexistent\n\tpushurl\n';
  const VALUELESS_PUSHURL_LINE = 5;

  describe('Given a config with a valued url and a valueless remote.origin.pushurl', () => {
    describe('When git push origin main is run', () => {
      it('Then git refuses with exit 128 reporting remote.origin.pushurl', async () => {
        // Arrange
        initRepo(ours);
        await writeFile(path.join(ours, '.git', 'config'), VALUELESS_PUSHURL_VALUED_URL_FIXTURE);
        ...
        expect(g.stderr).toContain(`at line ${VALUELESS_PUSHURL_LINE}`);
      });
    });
    ...
    describe('When tsgit push is run', () => {
      it('Then throws CONFIG_MISSING_VALUE with key remote.origin.pushurl and correct line', async () => {
        ...
        expect(data.line).toBe(VALUELESS_PUSHURL_LINE);
        expect(data.source).toMatch(/\/config$/);
      });
    });
    ...
    describe("When reconstructing git's two push lines from tsgit structured fields", () => {
      it("Then the reconstructed lines match git's stderr after path-token normalization", async () => {
        // Arrange
        initRepo(ours);
        await writeFile(path.join(ours, '.git', 'config'), VALUELESS_PUSHURL_VALUED_URL_FIXTURE);
    ...
    describe("When reconstructing git's two fetch lines from tsgit structured fields", () => {
      it("Then the reconstructed lines match git's stderr after path-token normalization", async () => {
        // Arrange
        initRepo(ours);
        await writeFile(path.join(ours, '.git', 'config'), VALUELESS_PUSHURL_VALUED_URL_FIXTURE);
  ...
  /**
   * Both `url` (line 4) and `pushurl` (line 5) valueless — git reports the FIRST
   * by file line (url), proving file-order ordering, NOT a `pushurl ?? url`
   * resolution order (P3).
   * Line 4: \turl              <- valueless (earlier)
   * Line 5: \tpushurl          <- valueless
   */
  const BOTH_VALUELESS_URL_EARLIER_FIXTURE =
    '[core]\n\trepositoryformatversion = 0\n[remote "origin"]\n\turl\n\tpushurl\n';
  const BOTH_VALUELESS_URL_LINE = 4;

  describe('Given a config with both url and pushurl valueless, url earlier', () => {
    describe('When git push origin main is run', () => {
      it('Then git refuses reporting the earlier key remote.origin.url', async () => {
        // Arrange
        initRepo(ours);
        await writeFile(path.join(ours, '.git', 'config'), BOTH_VALUELESS_URL_EARLIER_FIXTURE);
        ...
        expect(g.ok).toBe(false);
        expect(g.stderr).toContain("missing value for 'remote.origin.url'");
        expect(g.stderr).toContain(`at line ${BOTH_VALUELESS_URL_LINE}`);
      });
    });

    describe('When tsgit push is run', () => {
      it('Then throws CONFIG_MISSING_VALUE reporting the earlier key remote.origin.url', async () => {
        // Arrange
        initRepo(ours);
        await writeFile(path.join(ours, '.git', 'config'), BOTH_VALUELESS_URL_EARLIER_FIXTURE);
        const repo = await openRepository({ cwd: ours });
        ...
        expect(data.code).toBe('CONFIG_MISSING_VALUE');
        expect(data.key).toBe('remote.origin.url');
        expect(data.line).toBe(BOTH_VALUELESS_URL_LINE);
        expect(data.source).toMatch(/\/config$/);
      });
    });
  });
