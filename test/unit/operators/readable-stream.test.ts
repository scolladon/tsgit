...
describe('readableStreamToAsyncIterable', () => {
  describe('Given a stream yielding two chunks', () => {
    describe('When iterated', () => {
      it('Then both chunks surface in order', async () => {
        // Arrange
  ...
  describe('Given an immediately-closed stream', () => {
    describe('When iterated', () => {
      it('Then yields zero chunks and ends cleanly', async () => {
        // Arrange
        const stream = new ReadableStream<Uint8Array>({
          start(controller) {
            controller.close();
  ...
  describe('Given an early break', () => {
    describe('When consumer exits', () => {
      it('Then the iterator return cancels the underlying stream', async () => {
        // Arrange
        let cancelled = false;
        const stream = new ReadableStream<Uint8Array>({
          start(controller) {
  ...
  describe('Given a stream whose cancel() throws', () => {
    describe('When the iterator return runs', () => {
      it('Then the error is swallowed and the iterator still ends', async () => {
