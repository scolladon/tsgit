  ...
  it('Law: take(n) ∘ take(m) ≡ take(min(n, m))', async () => {
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (n, m, source) => {
          const composed = await toArray(take<number>(n)(take<number>(m)(toAsyncIterable(source))));
          const minimal = await toArray(take<number>(Math.min(n, m))(toAsyncIterable(source)));
          return composed.length === minimal.length && composed.every((v, i) => v === minimal[i]);
        },
      ),
      { numRuns: 50 },
    );
  ...
  it('Law: filter(p) ∘ filter(q) ≡ filter(x => q(x) && p(x))', async () => {
    ...
    await fc.assert(
      fc.asyncProperty(fc.array(fc.integer(), { maxLength: 30 }), async (source) => {
        ...
        const fused = await toArray(filter((x: number) => q(x) && p(x))(toAsyncIterable(source)));
