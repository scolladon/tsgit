...
describe('withLogging — events on success', () => {
  describe('Given inner resolves with statusCode 200', () => {
    describe('When request is awaited', () => {
      it('Then events contain request then response', async () => {
        // Arrange
  ...
  describe('Given a fakeClock that advances 250ms', () => {
    describe('When request is awaited', () => {
      it('Then response.elapsedMs equals 250', async () => {
        // Arrange
        const { logger, events } = recordingLogger();
...
describe('withLogging — events on failure', () => {
  describe('Given inner rejects', () => {
    describe('When awaited', () => {
      it('Then events are [request, error] and error reference unchanged', async () => {
        // Arrange
        const { logger, events } = recordingLogger();
  ...
  describe('Given a fakeClock that advances 700ms before inner rejects', () => {
    describe('When awaited', () => {
      it('Then error.elapsedMs equals 700', async () => {
        // Arrange
        const { logger, events } = recordingLogger();
...
describe('withLogging — header redaction', () => {
  describe('Given authorization (lowercase)', () => {
    describe('When sent', () => {
      it('Then events.headers.authorization is undefined', async () => {
        // Arrange
        const { logger, events } = recordingLogger();
  ...
  describe('Given Authorization (capital)', () => {
    describe('When sent', () => {
      it('Then NO key whose lowercase form is "authorization" is present', async () => {
        // Arrange
        const { logger, events } = recordingLogger();
        const { transport } = fakeTransport([makeResponse()]);
        const sut = withLogging({ logger })(transport);
  ...
  describe('Given redactHeaders=[]', () => {
    describe('When authorization present', () => {
      it('Then it is STILL stripped (forced; non-opt-out)', async () => {
        // Arrange
        const { logger, events } = recordingLogger();
        const { transport } = fakeTransport([makeResponse()]);
  ...
  describe('Given redactHeaders=["x-trace-id"]', () => {
    describe('When sent', () => {
      it('Then both authorization and x-trace-id are stripped', async () => {
        // Arrange
        const { logger, events } = recordingLogger();
        const { transport } = fakeTransport([makeResponse()]);
  ...
  describe('Given a response with set-cookie + authorization', () => {
    describe('When logged', () => {
      it('Then those headers are stripped from the response event', async () => {
        // Arrange
        const { logger, events } = recordingLogger();
  ...
  describe('Given a request with cookie, set-cookie, proxy-authorization, authorization headers', () => {
    describe('When sent', () => {
      it('Then all four are stripped', async () => {
        // Arrange
        const { logger, events } = recordingLogger();
        const { transport } = fakeTransport([makeResponse()]);
        const sut = withLogging({ logger })(transport);
...
describe('withLogging — URL redaction', () => {
  ...
  describe('Given url=%j', () => {
    describe('When logged', () => {
      it.each(cases)('Then events[0].url=%j', async (input, expected) => {
        // Arrange
        const { logger, events } = recordingLogger();
        const { transport } = fakeTransport([makeResponse()]);
        const sut = withLogging({ logger })(transport);
  ...
  describe('Given a URL with a non-sensitive query key', () => {
    describe('When logged', () => {
      it('Then that key is preserved (deletion list starts empty)', async () => {
...
describe('withLogging — extra header redaction defaulting', () => {
  describe('Given no redactHeaders config', () => {
    describe('When a non-sensitive header is sent', () => {
      it('Then it is preserved (extra-redact list defaults to empty)', async () => {
...
describe('withLogging — logger throw safety', () => {
  describe('Given a logger that throws on every event', () => {
    describe('When request resolves with 200', () => {
      it('Then wrapped resolves with 200 (throw swallowed)', async () => {
        // Arrange
        const logger: Logger = {
          log: () => {
  ...
  describe('Given a logger that throws only on request event', () => {
    describe('When sent', () => {
      it('Then response event still emitted and request resolves', async () => {
        // Arrange
