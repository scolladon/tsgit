...
describe('NodeFileSystem.existsContained — lean loose-probe (DI)', () => {
  const fileStat = {
    ctimeMs: BigInt(0),
    mtimeMs: BigInt(0),
    dev: BigInt(0),
  ...
  describe('Given a present regular file leaf', () => {
    describe('When existsContained is called', () => {
      it('Then returns true', async () => {
        ...
        // Act
        const result = await sut.existsContained('/root/objects/ab/leaf');
  ...
  describe('Given an absent leaf whose parent exists', () => {
    describe('When existsContained is called', () => {
      it('Then returns false', async () => {
        ...
        // Act
        const result = await sut.existsContained('/root/objects/ab/missing');
  ...
  describe('Given a leaf whose parent directory does not exist', () => {
    describe('When existsContained is called', () => {
      it('Then returns false and nothing is cached', async () => {
        // Arrange
        const rootDir = '/root';
        const realpathSpy = vi.fn().mockImplementation(async (input: string) => {
          if (input === rootDir) return rootDir;
        ...
        // Act
        const first = await sut.existsContained('/root/objects/ab/leaf');
        const second = await sut.existsContained('/root/objects/ab/other');

        // Assert — no cache write on ENOENT, so the parent is re-realpathed
        // on the second call too (mirrors the write/lstat slow-walk discipline).
        expect(first).toBe(false);
        expect(second).toBe(false);
        const parentCalls = realpathSpy.mock.calls.filter(
          ([arg]: readonly unknown[]) => arg === '/root/objects/ab',
        );
        expect(parentCalls.length).toBe(2);
      });
    });
  ...
  describe('Given a lexically out-of-tree path', () => {
    describe('When existsContained is called', () => {
      it('Then throws PERMISSION_DENIED', async () => {
        ...
        let caught: unknown;
        try {
          await sut.existsContained('/root/sub/../../escape/leaf.bin');
        } catch (err) {
          caught = err;
        }
  ...
  describe('Given an escaping-target leaf symlink', () => {
    describe('When existsContained is called', () => {
      it('Then returns true (present, leaf not followed)', async () => {
        // Arrange — the symlink itself lives inside rootDir (its lexical
        // path is contained); only its TARGET escapes. existsContained uses
        // lstat, which never dereferences the leaf, so it reports presence.
        const rootDir = '/root';
        const fsOps = fakeFsOps({
          realpath: vi.fn().mockImplementation(async (input: string) => input),
        ...
        // Act
        const result = await sut.existsContained('/root/objects/ab/escaping-link');

        // Assert — present. The subsequent read (case (c)) is the real gate.
        expect(result).toBe(true);
      });
    });

    describe('When the caller follows up with read', () => {
      it('Then read throws PERMISSION_DENIED (case (c) unchanged)', async () => {
        // Arrange
        const rootDir = '/root';
        const fsOps = fakeFsOps({
          realpath: vi.fn().mockImplementation(async (input: string) => {
            if (input === '/root/objects/ab/escaping-link') return '/outside/target';
            return input;
          }),
          lstat: vi.fn().mockResolvedValue({
            ...fileStat,
        ...
        // Act
        const probe = await sut.existsContained('/root/objects/ab/escaping-link');
        let caught: unknown;
        try {
          await sut.read('/root/objects/ab/escaping-link');
        } catch (err) {
          caught = err;
        }
  ...
  describe('Given a non-ENOENT errno on the parent realpath', () => {
    describe('When existsContained is called', () => {
      it('Then the error propagates (not swallowed as false)', async () => {
        // Arrange
        const rootDir = '/root';
        const fsOps = fakeFsOps({
          realpath: vi.fn().mockImplementation(async (input: string) => {
        ...
        let caught: unknown;
        try {
          await sut.existsContained('/root/objects/ab/leaf');
        } catch (err) {
          caught = err;
        }
  ...
  describe('Given a non-ENOENT errno on the leaf lstat', () => {
    describe('When existsContained is called', () => {
      it('Then the error propagates (not swallowed as false)', async () => {
        // Arrange
        const rootDir = '/root';
  ...
  describe('Given N existsContained probes sharing one fanout dir', () => {
    describe('When each probe fires', () => {
      it('Then realpath(fanout dir) is invoked exactly once (parent-cache reuse)', async () => {
        // Arrange — mirrors the object-resolver loose-probe shape: 5 sibling
        // leaves under the same fanout dir. This is the direct observable
        // that existsContained rides finding-3's cache and keeps the
        // packed-walk win.
        const rootDir = '/root';
        const realpathSpy = vi.fn().mockImplementation(async (input: string) => input);
        const fsOps = fakeFsOps({
        ...
        // Act
        for (let i = 0; i < 5; i += 1) {
          await sut.existsContained(`${fanoutDir}/leaf${i}`);
        }
  ...
  describe('Given the leaf lstat rejects with a non-errno value', () => {
    describe('When existsContained is called', () => {
      it('Then the original value rethrows unchanged (not swallowed, not mapped)', async () => {
        // Arrange — lstat rejects with a non-Error (string) so
        // isErrnoException returns false. The defensive rethrow keeps the
        // semantic that only errno faults flow through mapErrno.
        const rootDir = '/root';
        const fsOps = fakeFsOps({
          realpath: vi.fn().mockImplementation(async (input: string) => input),
          lstat: vi.fn().mockRejectedValue('not-an-error'),
        });
        const sut = new NodeFileSystem(rootDir, posixPolicy, fsOps);
