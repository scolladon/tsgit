...
describe('catFile', () => {
  ...
  describe('Given a malformed string id', () => {
    describe('When invoked', () => {
      it('Then throws INVALID_OBJECT_ID before any read', async () => {
        // Arrange
  ...
  describe('Given a mix of ObjectId and hex string inputs', () => {
    describe('When invoked', () => {
      it('Then both are accepted', async () => {
        // Arrange
        const ctx = await buildSeededContext();
        await ctx.fs.writeUtf8(`${ctx.layout.gitDir}/HEAD`, 'ref: refs/heads/main\n');
  ...
  describe('Given an empty ids array', () => {
    describe('When invoked', () => {
      it('Then returns { kind: "batch", entries: [] }', async () => {
        // Arrange
        const ctx = await buildSeededContext();
        await ctx.fs.writeUtf8(`${ctx.layout.gitDir}/HEAD`, 'ref: refs/heads/main\n');
        const sut = catFile;
  ...
  describe('Given maxBytes smaller than the stored blob', () => {
    describe('When invoked', () => {
      it('Then propagates OBJECT_TOO_LARGE (option forwarded to the primitive)', async () => {
        // Arrange
        const ctx = await buildSeededContext();
        await ctx.fs.writeUtf8(`${ctx.layout.gitDir}/HEAD`, 'ref: refs/heads/main\n');
  ...
  describe('Given maxBytes equal to the stored blob length', () => {
    describe('When invoked', () => {
  ...
  describe('Given a mix of stored and missing ids', () => {
    describe('When invoked', () => {
      it('Then per-entry ok shape in input order', async () => {
        // Arrange
        const ctx = await buildSeededContext();
        await ctx.fs.writeUtf8(`${ctx.layout.gitDir}/HEAD`, 'ref: refs/heads/main\n');
