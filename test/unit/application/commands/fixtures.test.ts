...
describe('commands/fixtures', () => {
  describe('seedRepo', () => {
    it('Given seed with one commit + working tree, When seedRepo, Then commit is readable and HEAD points to refs/heads/main', async () => {
      // Arrange
    ...
    it('Given seed with explicit head as a 40-hex oid, When seedRepo, Then HEAD is detached (oid only, no ref: prefix)', async () => {
      // Arrange
      const ctx = createMemoryContext();
  ...
  describe('memoryRemote', () => {
    it('Given an advertisement, When request to info/refs?service=git-upload-pack, Then returns pkt-line-framed advertisement', async () => {
      // Arrange
    ...
    it('Given a packBody, When request to git-upload-pack POST, Then response contains NAK + sideband-1 packed bytes', async () => {
      // Arrange
  ...
  describe('recordedTransport', () => {
    it('Given a wrapped transport, When two requests are issued, Then requests array has 2 entries in order', async () => {
      // Arrange
      const recorder = recordedTransport();

      // Act
      await recorder.transport.request({ method: 'GET', url: 'https://a.example/', headers: {} });
    ...
    it('Given an inner transport, When request, Then inner is called and recorded', async () => {
      // Arrange
