...
describe('diff', () => {
  ...
  it('Given `from` omitted, When diff with only `to`, Then resolves the missing side as HEAD (not the empty string)', async () => {
  ...
  it('Given `to` omitted, When diff(from=HEAD), Then diffs HEAD tree against the empty tree', async () => {
    // Arrange
    const ctx = createMemoryContext();
    await init(ctx);
    await ctx.fs.writeUtf8(`${ctx.layout.workDir}/a.txt`, 'a1');
  ...
  it('Given a rename and detectRenames=true, When diff, Then collapses delete+add into a single rename change', async () => {
  ...
  it('Given a rename and detectRenames omitted, When diff, Then yields separate add and delete (no rename)', async () => {
  ...
  it('Given detectRenames=false explicitly, When diff on a rename, Then no rename change is produced', async () => {
    // Arrange
    const ctx = createMemoryContext();
    await init(ctx);
    const content = 'unique content used to anchor rename detection';
  ...
  it('Given a tree oid (not a commit) as `from`, When diff, Then resolves the tree id verbatim', async () => {
  ...
  it('Given a tree oid vs the empty tree, When diff, Then a non-commit target still produces real changes', async () => {
  ...
  it('Given a 41-hex-character `from`, When diff, Then it is treated as a ref name — proving both regex anchors', async () => {
    // Arrange
    const ctx = createMemoryContext();
    await init(ctx);
    await ctx.fs.writeUtf8(`${ctx.layout.workDir}/a.txt`, 'a1');
  ...
  it('Given a `from` with a 39-hex prefix and a non-hex suffix, When diff, Then it is treated as a ref name', async () => {
    // Arrange
    const ctx = createMemoryContext();
    await init(ctx);
    await ctx.fs.writeUtf8(`${ctx.layout.workDir}/a.txt`, 'a1');
  ...
  it('Given an explicit HEAD target, When diff(from=HEAD), Then resolves the HEAD ref', async () => {
    // Arrange
    const ctx = createMemoryContext();
    await init(ctx);
    await ctx.fs.writeUtf8(`${ctx.layout.workDir}/a.txt`, 'a1');
  ...
  it('Given a branch ref name as target, When diff, Then resolves it via validateRefName', async () => {
  ...
  it('Given a non-existent ref name as target, When diff, Then fails ref resolution (proving the non-HEAD branch is taken)', async () => {
    // Arrange
    const ctx = createMemoryContext();
    await init(ctx);
    await ctx.fs.writeUtf8(`${ctx.layout.workDir}/a.txt`, 'a1');
