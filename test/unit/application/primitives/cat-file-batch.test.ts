...
describe('catFileBatch', () => {
  describe('basics', () => {
    it('Given an empty iterable, When iterated, Then yields no entries', async () => {
      // Arrange
    ...
    it('Given a stored blob id, When iterated, Then yields one ok entry with type/size/object', async () => {
      // Arrange
      const ctx = await buildSeededContext();
    ...
    it('Given a sync iterable of ids, When iterated, Then yields entries in order', async () => {
      // Arrange
      const ctx = await buildSeededContext();
    ...
    it('Given an async iterable of ids, When iterated, Then yields entries equivalently', async () => {
      // Arrange
      const ctx = await buildSeededContext();
  ...
  describe('size per object type', () => {
    it('Given a stored tree id, When iterated, Then size equals serialized tree body length', async () => {
      // Arrange
      const ctx = await buildSeededContext();
    ...
    it('Given a stored commit id, When iterated, Then size equals serialized commit body length', async () => {
      // Arrange
      const ctx = await buildSeededContext();
    ...
    it('Given a stored tag id, When iterated, Then size equals serialized tag body length', async () => {
      // Arrange
      const ctx = await buildSeededContext();
      const blobId = await writeBlobBytes(ctx, new Uint8Array([1]));
  ...
  describe('missing ids', () => {
    it('Given a missing id, When iterated, Then yields { ok: false, reason: "missing" }', async () => {
      // Arrange
      const ctx = await buildSeededContext();
    ...
    it('Given mixed hits and misses, When iterated, Then yields per input id in input order', async () => {
      // Arrange
      const ctx = await buildSeededContext();
  ...
  describe('error propagation', () => {
    it('Given a non-OBJECT_NOT_FOUND TsgitError thrown by the read, When iterated, Then propagates the error', async () => {
      // Arrange
      const ctx = await buildSeededContext();
    ...
    it('Given a non-TsgitError thrown by the read, When iterated, Then propagates the error', async () => {
      // Arrange
      const ctx = await buildSeededContext();
      const stored = await writeBlobBytes(ctx, new Uint8Array([1]));
      const probe: Context = {
  ...
  describe('cancellation', () => {
    ...
    it('Given a signal aborted after the first yield, When iteration continues, Then the next pull throws OPERATION_ABORTED', async () => {
      // Arrange
      const controller = new AbortController();
      const ctx = await buildSeededContext({ signal: controller.signal });
      const a = await writeBlobBytes(ctx, new Uint8Array([1]));
