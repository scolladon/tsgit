...
describe('primitives/write-sparse-checkout', () => {
  describe('Given pattern text', () => {
    describe('When writeSparsePatternText', () => {
      it('Then the file is written at .git/info/sparse-checkout', async () => {
        // Arrange
  ...
  describe('Given an existing pattern file', () => {
    describe('When writeSparsePatternText', () => {
      it('Then the file is overwritten', async () => {
        // Arrange
        const ctx = createMemoryContext();
  ...
  describe('Given a fresh repository', () => {
    describe('When writeSparsePatternText', () => {
      it('Then .git/info is mkdir-ed before the write', async () => {
        ...
        const writeIndex = ops.findIndex(
          (c) => c.method === 'writeUtf8' && c.path === sparsePath(ctx),
        );
