...
describe('computeChangeset', () => {
  it('Given an empty index and empty target tree, When computeChangeset runs, Then returns no entries', () => {
    // Arrange
  ...
  it('Given an empty index and a tree with one blob, When computeChangeset runs, Then emits one add', () => {
    // Arrange
    const sut = computeChangeset;
  ...
  it('Given an index with one entry and empty target tree, When computeChangeset runs, Then emits one delete', () => {
    // Arrange
    const sut = computeChangeset;
  ...
  it('Given an index entry and target entry with the same oid and mode, When computeChangeset runs, Then emits one noop', () => {
    // Arrange
    const sut = computeChangeset;

    // Act
    const result = sut(makeIndex([makeEntry('foo.txt', OID_A)]), [makeTreeEntry('foo.txt', OID_A)]);
  ...
  it('Given an index entry and a target entry with a different oid, When computeChangeset runs, Then emits one update', () => {
    // Arrange
    const sut = computeChangeset;

    // Act
    const result = sut(makeIndex([makeEntry('foo.txt', OID_A)]), [makeTreeEntry('foo.txt', OID_B)]);
  ...
  it('Given an index entry and a target entry with same oid but different mode, When computeChangeset runs, Then emits one update', () => {
    // Arrange
    const sut = computeChangeset;
  ...
  it('Given mixed paths across index and target tree, When computeChangeset runs, Then result entries are sorted by path', () => {
    // Arrange
    const sut = computeChangeset;
  ...
  it('Given an index with a non-stage-0 entry, When computeChangeset runs, Then ignores the non-stage-0 entry', () => {
    // Arrange
    const sut = computeChangeset;
  ...
  it('Given symlinks and gitlinks in the target tree, When computeChangeset runs, Then preserves the mode through entries', () => {
    // Arrange
    const sut = computeChangeset;
