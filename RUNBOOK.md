...
`docs/understand/performance.md` (the full table plus the `captured <date>` provenance
line) — the single published comparison surface; the README only links to it (ADR-624) —
so the citable numbers stay honestly dated. This is a manual step, not a gate — the
numbers are host-specific artefacts, transcribed by a human, never auto-committed.
