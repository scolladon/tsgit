...
- **Change part 2 (valueless guard) — UPDATE-PRIORITY ordering (empirically pinned, supersedes the earlier "single joint by-line call" idea):** a valueless `submodule.<n>.update` parses to `value: null` (skipped by `mergeSubmodule`, so `config.submodule[name].update === undefined`), so the typed read alone cannot distinguish valueless from absent — the guard catches it. `update` is read for EVERY row, not only url-undefined, so its guard sits at the row-body top. **CRITICAL git-faithfulness finding (pinned against git 2.54.0, both orders + url-absent + url-only):** `git submodule update` reads `submodule.<n>.update` BEFORE `url` and reports `submodule.<n>.update` with STRICT PRIORITY regardless of file-line order — even when `url` is valueless on an *earlier* line, and even when `url` is absent — reporting `url` only when `update` is valued/absent. A single by-line `['url','update']` call (the original plan idea) would DIVERGE (it reports the earlier-line `url`). **Faithful shape = two priority-ordered guards:** add an unconditional `await assertNoValuelessConfig(ctx, 'submodule', row.name, ['update']);` at the TOP of the per-row body (fires first → update-priority); LEAVE slice 3's existing `['url']` guard on the url-undefined branch UNTOUCHED (fires only when `update` is valued/absent and `url` is valueless). Do NOT merge them into one call; do NOT move slice 3's url guard. Verify: valued url + valued update → both no-op; the url-undefined branch logic and the new mode resolution run unchanged after the update guard.
  - **Caution:** the row-body-top `['update']` guard fires before any per-row init — matches git (git validates `update` before acting). Re-run slice 3's `url` interop pins to confirm no regression (they are unchanged: slice 3's guard stays put).
...
7. **RED (unit, url+update co-occurrence — update-priority, BOTH orders report `update`):** url-then-update valueless → `submodule.mysub.update` (NOT url, despite url's earlier line); update-then-url valueless → `submodule.mysub.update`; AND url-valueless + update-absent → `submodule.mysub.url` (the url path still works when update is not valueless). Pinned: git reports `update` with priority regardless of line order.
8. **GREEN:** add `resolveUpdateMode` helper, replace line 722; add the unconditional row-body-top `['update']` guard; LEAVE slice 3's `['url']` guard on the url-undefined branch untouched (do not merge or move it).
9. **RED→GREEN (interop):** precedence matrix block (compare HEAD vs git) + valueless `update` row + the update-priority co-occurrence pin (both orders → `submodule.mysub.update`; url-only → `submodule.mysub.url`). Re-run slice 3's `url` interop pins (unchanged — slice 3's guard stays put).
...
## Slice 7 — `core.hooksPath` PER-ACCESSOR valueless refusal at hook resolution (ADR-350)

> **DECISION CHANGED (ADR-350, refines ADR-346).** Empirical pinning showed `core.hooksPath`'s death breadth is intricate, ruleless, and flag-dependent (dies on status/log/add/diff/show/commit/checkout/merge/name-rev/blame/describe/reflog; survives on branch/tag/cat-file/rev-parse/shortlog/mv/rm/stash/reset/no-op-rebase/range-diff/whatchanged/worktree/sparse-checkout/fetch/pull/push; a committing cherry-pick dies but `--no-commit` survives). A static per-command gate cannot reproduce per-invocation behaviour. **The user chose the per-accessor mechanism** (gate at tsgit's hook-resolution point, mirroring git's `find_hook`), accepting a documented under-refusal. This slice does NOT extend `assertOperationalRepository` and does NOT add a `{ hooks }` option — that prior idea is RETIRED.

### Context
- **Placement (ADR-350): gate at the hook-resolution point.** tsgit reads `config.core?.hooksPath` to locate the hooks dir in `run-hook.ts` (`invokeHook` → `resolveHooksDir(config.core?.hooksPath, …)`, ~line 45-63). Add `await assertNoValuelessConfig(ctx, 'core', undefined, ['hookspath'])` at the point hookspath is resolved, so tsgit refuses when a command looks for a hook with a valueless `hookspath` — mirroring git's `find_hook` → `git_config_get_pathname` death. **Inspect `run-hook.ts` first** (read both `runHook` and the informational `runInformationalHook` from backlog 24.8): the guard must cover EVERY hook-resolution path (blocking and informational), so place it at the shared resolution point (likely in/just before `resolveHooksDir`, or a shared helper both call) — NOT only on the blocking path. git dies during pathname resolution BEFORE checking whether a hook file exists, so the guard must fire whenever a command resolves the hooks dir, even if no hook file is present. Verify tsgit's resolution does not short-circuit when the hook file is absent; if it does, ensure the hookspath read (and thus the guard) still runs at the lookup point so a valueless `hookspath` refuses on every hook-running command (commit/merge/checkout/working-rebase/cherry-pick/revert/pull-via-merge), matching git on those.
- **Import:** `run-hook.ts` is a primitive (`src/application/primitives/`); import `assertNoValuelessConfig` from `./internal/valueless-config-guard.js` (same-tier primitive — legal after slice 1's relocation).
- **`assertOperationalRepository` / slice 6 stay UNCHANGED** — they carry `['excludesfile','attributesfile']` only. Do NOT add `hookspath` to them; do NOT touch the command-switch from slice 6.
- **Documented under-refusal (ADR-350):** tsgit will NOT refuse on `log`/`blame`/`diff`/`show`/`name-rev`/`describe`/`reflog` where git dies incidentally — those commands resolve no hook in tsgit. This is the conscious, recorded divergence. The interop pins AGREEMENT only on hook-resolving commands; it documents (does not assert agreement on) the under-refusal commands.
- **Test files:**
  - Unit: `run-hook.ts`'s test file (find it — likely `test/unit/application/primitives/run-hook.test.ts`). Add: valueless `hookspath` + a command path that resolves a hook → `CONFIG_MISSING_VALUE { key: 'core.hookspath', line: <N> }` (try/catch + `.data`); valued `hookspath` → resolves to that dir, no throw; absent `hookspath` → resolves to `${gitDir}/hooks` default, no throw. Cover BOTH `runHook` and `runInformationalHook` resolution paths (isolated tests).
  - Interop: extend `missing-value-refusal-interop.test.ts`. **Hook-resolving agreement (load-bearing):** valueless `hookspath` fixture → assert a hook-running command (`repo.commit` is simplest; optionally `repo.merge`/`repo.checkout`) DIES with `CONFIG_MISSING_VALUE` in tsgit AND `git commit` dies in git (two-line reconstruction). **Under-refusal documentation:** assert tsgit `repo.branch.list()` (and optionally `repo.log`) SURVIVES the valueless `hookspath` (does NOT throw) — pinning tsgit's per-accessor boundary; do NOT assert git agreement there (git's `log` dies — the documented ADR-350 divergence). Config porcelain (`configList`/`configGet`) survives. Absent `hookspath` → command proceeds (default hooks dir). Light per-case `beforeEach` tmpdir.

### TDD steps
1. **RED (unit, valueless hookspath at hook resolution):** seed HEAD + `[core]\n\thookspath\n` + a context that invokes a hook lookup; call the hook-resolution path → `CONFIG_MISSING_VALUE { key: 'core.hookspath', line: <N> }`. FAILS now: `resolveHooksDir` treats valueless as `undefined` → `${gitDir}/hooks` default, no throw.
2. **RED (unit, informational-hook path also refuses):** the `runInformationalHook` resolution path on the same fixture → also `CONFIG_MISSING_VALUE` (isolated test — proves both paths guarded).
3. **RED (unit, valued hookspath resolves):** `[core]\n\thookspath = /custom/hooks\n` → resolves to `/custom/hooks`, no throw.
4. **RED (unit, absent hookspath → default):** no `[core]` → resolves to `${gitDir}/hooks`, no throw. Pins the absent vs valueless distinction.
5. **GREEN:** add the import + the `['hookspath']` guard at the shared hook-resolution point in `run-hook.ts`.
6. **RED→GREEN (interop):** hook-resolving agreement (`repo.commit` dies in tsgit + `git commit` dies, reconstruction) + under-refusal documentation (`repo.branch.list()` survives in tsgit) + config porcelain survives + absent → proceeds.
7. **REFACTOR:** the guard is one awaited statement at the resolution point; no nesting >2; both hook paths share it (no duplication).
...
(adjust the run-hook test path if the file lives elsewhere; also `npx depcruise src/ --config .dependency-cruiser.cjs` to confirm the primitive→primitive guard import is clean)

### Commit
`feat(config): refuse valueless core hooksPath at hook resolution`
