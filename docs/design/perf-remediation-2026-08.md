...
- **R4** F2's cursor-based path descent preserves `parseTreeContent`'s duplicate-entry-name
  ...
  directory (ADR-723; no divergence is taken). **Superseded:** the duplicate-entry-name half of
  R4 no longer holds — ADR-752 drops that refusal from the read paths after pinning that git
  accepts duplicates everywhere except `fsck`. The eager mode validation stands.
...
> **That probe was run, and it settled the question against this section.** git accepts duplicate
> entry names on every read surface — `ls-tree` lists both, `rev-parse` resolves the first,
> `read-tree` keeps the last, `diff-tree` emits both — and refuses only in `fsck`. ADR-752
> supersedes ADR-723 for the read-path refusal and removes the per-directory `Set` described
> above. Everything else in this section stands.
...
this note called for. Re-measured on the same three corpora: the two synthetic, effectively
all-reachable ones stay pinned against `git -c pack.threads=1 repack -a -d -f` (`-f` forces a
fresh delta search rather than reusing whichever deltas the corpus's own pack already carries) —
**×1.58** on the barely-deltifiable corpus and **×5.43** on the deep-delta-chain corpus, the shape
where `buildPack`'s window ordering by size, not git's path/name-hash, bites hardest (a long run
of same-size versions of one file ties under size ordering, so the window samples the wrong
...
delta bytes at the margin — both brackets land unchanged. tsgit's own real history genuinely
carries unreachable objects that `repack -a -d` (no `-A`) drops, so its peer is
`git -c pack.threads=1 gc` instead: both `gc`s retain unreachable objects newer than the two-week
expiry, landing the two sides on comparable object counts (24 879 git, 24 817 tsgit, within
0.25%) instead of charging tsgit for objects git already deleted — **×1.42**, not `×1.00`, for the
same size-vs-name-hash ordering reason. See `docs/use/commands/maintenance.md` §"The size trade"
for the class a caller should plan against.
...
- ~~**A delta-capable pack writer.**~~ **Shipped** — `docs/design/delta-writing-packer.md`
  ...
  re-emits existing delta chains as full bases. Re-measured on the same corpora: **×1.58**
  barely-deltifiable and **×5.43** worst-case deep-delta-chain against
  `git -c pack.threads=1 repack -a -d -f`, **×1.42** real history against `git -c pack.threads=1
  gc` — real history carries unreachable objects that `repack -a -d` drops but both `gc`s retain,
  so `gc` is the peer that keeps object counts comparable (§the size trade, measured) — down from
  the **×1.29 … ×6.91** base-only brackets this row predicted. `*.keep` (Pin V) remains
