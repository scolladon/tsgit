...
| `core.hooksPath` | **intricate, ruleless, flag-dependent** (pinned later than the rest) — dies on `status`/`log`/`add`/`diff`/`show`/`commit`/`checkout`/`merge`/`name-rev`/`blame`/`describe`/`reflog`; **survives** on `branch`/`tag`/`cat-file`/`rev-parse`/`shortlog`/`mv`/`rm`/`stash`/`reset`/`rebase`/`range-diff`/`whatchanged`/`worktree`/`sparse-checkout`/`fetch`/`pull`/`push`; flag-dependent (`cherry-pick --no-commit` survives, a committing cherry-pick dies). No clean rule — git dies wherever a code path incidentally resolves the hooks dir. | **YES (per-invocation)** | 128 | `core.hookspath` (lowercased) | its line | NO — does **not** fall back to `.git/hooks` default. Config porcelain survives. **tsgit: per-accessor gate at hook resolution (ADR-350), a documented under-refusal — NOT the eager-broad gate.** |
...
- **`git remote` is NOT a uniform-breadth command (matrix gap found at implement time, pinned git 2.54.0).** Its verbs split: `remote` (list)/`-v`, `get-url`, `add`, `rename`, `set-url`, `remove`, and `remote show -n` all **survive** a valueless `core.excludesfile`/`attributesfile`; only the **networked** `remote show <n>` (without `-n`) dies. tsgit's `remoteShow` is **local-only** (reads config + the local ref store + branch referrers, no transport), so its faithful peer is `git remote show -n` — which survives. **Resolution: ALL tsgit `remote` verbs stay on bare `assertRepository`** (none take the operational gate); gating `remoteShow` would over-refuse where the local `remote show -n` lives. This is faithful, not a divergence (tsgit has no networked remote-show to match git's dying one).
...
- **`core.excludesFile` / `core.attributesFile`** *(DECIDED — ADR-346, EAGER BROAD GATE)* — git's death is **eager and broad** (pinned: they die on the whole porcelain incl. `log`/`branch`/`tag`/`for-each-ref`). A per-accessor guard would leave `tsgit log` succeeding where `git log` dies — an observable refusal-condition divergence the prime directive forbids. **Placement (ADR-346): a shared eager gate** over `['excludesfile','attributesfile']`, run before the command does its work, on the operational surface but NOT the config porcelain. See the chokepoint analysis below.
- **`core.hooksPath`** *(DECIDED — ADR-350, PER-ACCESSOR, refines ADR-346)* — its death breadth is intricate, ruleless, and flag-dependent (see the matrix row), so a static per-command eager gate cannot reproduce it. **Placement (ADR-350): a per-accessor guard at tsgit's hook-resolution point** — `run-hook.ts` `invokeHook` → where `config.core?.hooksPath` is read to locate the hooks dir (`resolveHooksDir`), mirroring git's `find_hook` mechanism. tsgit refuses when a command resolves the hooks dir with a valueless `hookspath` (commit/merge/checkout/rebase-cherry-pick-revert hook points/pull-via-merge — matching git on those). A **documented, conscious under-refusal** is accepted: git also dies incidentally on `log`/`blame`/`diff`/`show`/`name-rev`/`describe`/`reflog`; tsgit does not. `hookspath` is NOT added to the `assertOperationalRepository` broad gate, and `excludesfile`/`attributesfile` are NOT added to the hook-resolution guard.
...
- **`hookspath` is OUT of the eager gate (ADR-350).** Empirical pinning showed its breadth is intricate, ruleless, and flag-dependent (matrix row) — no clean surface to gate. So the eager `assertOperationalRepository` gate validates only `['excludesfile','attributesfile']`. `hookspath` is handled separately, per-accessor at `run-hook.ts`'s hook-resolution point (ADR-350), accepting a documented under-refusal. Do NOT add `hookspath` to the broad gate; do NOT add `excludesfile`/`attributesfile` to the hook-resolution guard.

This chokepoint is the design's recommendation: the eager `assertOperationalRepository` gate carries `['excludesfile','attributesfile']` only (proven by the interop breadth matrix — multiple operational commands die + `config --list` survives), and `hookspath` is a separate per-accessor guard at hook resolution (ADR-350).
...
**Cross-key file-line ordering.** When two keys under one subsection are both valueless, git's reported key follows ONE of two regimes, pinned empirically per site:

- **File-line order (the common regime).** For `branch.<n>.remote`/`.merge`, `remote.<n>.pushurl`/`.url`, `merge.<d>.driver`/`.name`, and the `[core]` path-likes, git reports whichever key is **earlier in the config file** (its per-entry config-callback order). A single `assertNoValuelessConfig(ctx, section, sub, [k1, k2, …])` call reproduces this — `findFirstValuelessEntry` scans the token stream by line and returns the first match across all keys. This makes the **single multi-key call** the faithful shape for these sites (ADR-348 `['remote','merge']`, ADR-349 `['driver','name']`/`['pushurl','url']`, the `[core]` gate's `['excludesfile','attributesfile','hookspath']`).
- **Field-priority order (the `submodule.<n>` exception).** `git submodule update` does NOT use file-line order for `url`+`update`: it reads `submodule.<n>.update` BEFORE `url`, so it reports `submodule.<n>.update` with strict priority **regardless of line order** — even when `url` is valueless on an earlier line, and even when `url` is absent — and reports `url` only when `update` is valued/absent. Pinned against git 2.54.0 (mktemp `file://` submodule, both orders + url-absent + url-only). A single by-line `['url','update']` call would diverge (it would report the earlier-line `url`). The faithful shape is therefore **two priority-ordered guards**: an unconditional row-body-top `['update']` guard (fires first → update-priority), then slice 3's existing `['url']` guard on the url-undefined branch (fires only when `update` is valued/absent). Slice 3's `url` placement is left UNTOUCHED; slice 4 only ADDS the `['update']` guard before it.
...
- **`merge.<d>.driver`/`.name` on an UNUSED driver** — documented under-refusal (pinned: git's merge machinery loads ALL `[merge "*"]` sections at merge time, so a valueless `merge.<d>.name`/`.driver` dies even when no path selects driver `<d>`; tsgit's `namedChoice` guard fires only when the driver IS selected for a conflicting path). This is the same lazy-vs-eager shape as the `hookspath` per-accessor under-refusal (ADR-350) and consistent with the project's lazy/cold-path default (ADR-327): the common case (driver selected) is faithful; the configured-but-unused-driver edge is a bounded, recorded divergence. A follow-up could validate all merge-driver sections at merge time if needed.
