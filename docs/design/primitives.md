...
- **`verifyHash` defaults to `false`**, matching canonical git: an ordinary read does not re-hash on every object access, and this is the largest single read-path CPU cost when left on unconditionally. Callers that need to detect corruption opt in with `verifyHash: true` (`fsck` and `bundle verify` do). A mismatched hash throws `OBJECT_HASH_MISMATCH`.
