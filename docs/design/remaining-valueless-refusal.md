...
ADR-347's decision text names `namedChoice`; M4 was not in the matrix ADR-347 saw. This is a **load-bearing divergence between an accepted ADR and the pinned behaviour** — I cannot silently re-place the guard, and cannot silently keep a placement the matrix proves unfaithful. It is escalated as the single blocking item below; the design defaults to **(A)** as the faithful-maximal-consistent choice but does not enact it without the user confirming the ADR-347 amendment. (If (A) is chosen, ADR-347 is amended: "guard at the content-merge-table load in `buildContentMerger`, scanning all `[merge *]` subsections" — the lazy/merge-scoped intent is preserved; only the chokepoint moves up from `namedChoice` to the merger that calls it.)
...
| `merge.*.driver`/`name` | **Lazy** at custom-driver resolution (ADR-347). | **Pinning forces a placement correction** — see the blocking escalation. Default: option (A), the content-merge-table load in `buildContentMerger`, all `[merge *]` subsections (M4); `namedChoice` as written (option B) is unfaithful to M4. |

### Blocking escalation (ADR-347 placement vs pinned M4)

```
{ phase: design,
  reason: "ADR-347 fixes the merge-driver guard at `namedChoice` (reached only when a `merge=<name>` attribute resolves a path). Re-pinning under an isolated env (M4) shows git dies on a valueless `merge.*.driver`/`name` during ANY real 3-way content merge — including a conflicting `git merge` and `git merge-tree` with NO `.gitattributes` referencing the driver. A `namedChoice`-only guard misses the common no-attribute case, so the ADR-347 placement as written is unfaithful to the maximal scope it was accepted under.",
  options: [
    "(A) Amend ADR-347: move the guard up to the content-merge-table load in `buildContentMerger` (the chokepoint all 3-way consumers route through), scanning ALL `[merge *]` subsections, fired once-lazily on the first content merge. Faithful to M1/M2/M4, preserves ADR-347's lazy/merge-scoped intent; slightly larger than a one-line `namedChoice` edit and needs the new wildcard scan (§0, already needed for `branch`).",
    "(B) Keep ADR-347 verbatim — guard only at `namedChoice`, resolved-subsection. Smallest diff, but a documented faithfulness gap: a conflicting merge with no custom `merge=` attribute does NOT refuse, where git does (M4). Inconsistent with ADR-346 faithful-maximal.",
    "(C) Defer `merge.*` to a follow-up under the corrected matrix, ship the other three families now. Keeps this PR's merge path untouched until the placement is decided."
  ]
}
```
...
- **`merge` lazy guard (placement (A), pending the escalation)**: conflicting 3-way merge with valueless `merge.<d>.driver`, **no attribute** → `{ key:'merge.<d>.driver', line }` (M4); valueless `name` (driver valued) → `.name` (M2); clean / fast-forward merge with the same valueless driver → no throw (M3 laziness); a cherry-pick/revert reaching the same content merger inherits the throw (one representative, others by shared-path note); valid driver → resolves.
...
- `merge.custom.driver` valueless, **no attribute** → `git merge <conflicting>` vs tsgit `merge` (M4) — pending the ADR-347 amendment.
