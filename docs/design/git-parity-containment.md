...
[ADR-051](../adr/051-symlink-target-containment.md) (symlink target containment),
...
- **R2** — Every write-side surface (`write`, `writeStream`, `writeExclusive`, `writeUtf8`,
  ...
  `openWithNoFollow(_, 'write')`) carries a symlink-aware guard. No write surface loses a guard it has
  today, and the two that are guarded by a *read-shaped* mode today (`chmod`, `rename` src) gain a
  write-shaped one.
...
- **R5** — Working-tree **content** reads never dereference a symlink leaf. The six sites, audited
  individually: `apply-changeset.ts:62`, `compare-working-tree-entry.ts:118` (ternary today),
  `snapshot/workdir-entry.ts:80` (ternary today), `blame.ts:184` (ternary today), `grep.ts:82`,
  `stash.ts:124` (ternary today). Each must reach `ctx.fs.read` only on a path it has already
  established is not a symlink; the four ternaries document the pattern the other two are measured
  against.
...
- **R7** — Blast radius is the Node adapter. Memory and Browser/OPFS behaviour is unchanged.
...
| `symlink` (absolute target) | `realpathNearestExisting` + root set (ADR-051) | unchanged — and now load-bearing (§7.4) | DC-10 |
...
Preserved unchanged: **every** `PERMISSION_DENIED` on a write escape (R2), the facade's
...
| Working-tree entry is a symlink to `/etc/passwd` (absolute) | stores/writes the link; never dereferences | same; `add` stores the target as blob bytes | a caller doing a blind `ctx.fs.read` on it would dereference → R5 audit of the six sites |
| Working-tree entry is a symlink to `../../../etc/passwd` (relative) | same | same; **ADR-051 exempts relative targets**, so the link can be created | same as above; this is the one scenario where today's read gate *did* help. Enumerated, mitigated by R5, recorded in DC-10 |
...
| P9 | `walkWorkingTree`'s per-entry `lstat` (`walk-working-tree.ts:103`) | Real waste: `walkInternal`'s `readdir` already returned `isFile`/`isDirectory`/`isSymbolicLink` for that entry, and `status`'s untracked pass (`status.ts:206`) reads only `{ path }` off each entry — the `lstat` and its second `joinPath` are pure cost there. Not a containment change → **DC-11** |
| P10 | `status`'s double `lstat` per tracked path | `compareWorkingTreeDelta:90` lstats every stage-0 entry and `visitEntry:103` lstats the same path again during the untracked walk. Deduping needs a shared stat map across the two passes → **DC-12** |
...
Honest expectation per scenario: `status:clean` is dominated by the per-entry containment share and
should move the most. `readBlob:cold` and `delta-chain:cold` are dominated by **repository-open fixed
cost** (discovery, pack-registry load, `.idx`/`.rev`/midx probes) of which containment realpaths are
one component per artefact probe — the win is real but bounded by the rest of the open path, and P5's
`exists` collapse is the largest single contributor there.

### §11 Superseded and refined

| Artefact | Disposition |
|---|---|
| [`docs/understand/security.md`](../understand/security.md) §"Path containment" — *"Every `FileSystem` adapter enforces that every input path resolves to a location inside one of the adapter's containment roots"*, the three-mode realpath table, and *"symlinks pointing outside every root … throw `PERMISSION_DENIED`"* | **Superseded** for the Node adapter's read side. Must be rewritten to the read/write asymmetry, keeping the Memory and Browser paragraphs as-is |
...
| [**ADR-051**](../adr/051-symlink-target-containment.md) (symlink target containment) | **Retained and re-motivated** — it becomes the primary creation-time defence once reads are unguarded (§7.3). Its relative-target exemption is the enumerated residual. DC-10 |
...
| **Stays — write side** | `node-file-system.test.ts` L97 (write onto a symlink leaf), L1429 (absolute symlink target); `injected` L1958 (×2, creation post-check), L2214/L2246 (symlink target), L666/L2916 (Windows symlink refusal), L2276 (relative target exempt); `interpretCreationLstat`'s four cases; `posix-only/node-fs-real-symlinks.test.ts` cases 1 and 3; `win-only/node-fs-windows-real.test.ts` case 2 | keep the expectation; re-point at W1/W2. Where `O_NOFOLLOW` replaces the pre-write `lstat`, the injected test asserts the **flag** passed to `open`/`writeFile`, not an absent `lstat` |
...
the **link** and leaves the target in place.
...
`src/domain/git-index/path-validator.ts` per the repo's Stryker procedure. The four benchmark
scenarios are read off the CI nightly, never a local run.
...
- **Relaxing write refusals git does not have** — the symlink-leaf refusal on `appendUtf8` (a
  symlinked `.git/logs/HEAD` would be followed by git) and ADR-051's absolute-target refusal are
  pre-existing, stricter-than-git write behaviours. The brief's mandate is to *preserve* write
  refusals; relaxing them is a separate faithfulness item (DC-10 records the trade).
