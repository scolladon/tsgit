...
stage 2's size direction on this corpus is **unknown**, not a deliberate regression. Promotion
(stage 3), measured, confirms half of this claim and falsifies the other half. On
`DELTA_CHAIN` — a single straight chain — it changes nothing: the predecessor is already the
most recent member, and the S3 row is byte-identical to S2 in every field. On
`MEDIUM_FIXTURE` it is **not** a no-op: its trees are not one chain but a forest where the same
base repeatedly serves several targets, and promotion's reordering of which base is tried
first moves 11 trees from base to delta, shortens the deepest chain by one level, and still
grows the pack by 1,845 bytes — see the S3 row in §11b.
...
| **S2** | depth-scaled bound (§4h) | which base wins at depth | **measured, git 2.55.0 — the structural prediction confirmed; the ratio crosses from above to below parity.** ×0.97 (ours 181,396 B vs peer 187,097 B; 900/900 objects), down from S1b's ×1.07. Blob base rises from 6 to **7** — matching peer's own count exactly — delta falls from 294 to 293 (peer: 293), and maxDepth drops off the cap from **50** to **44**, inside git's own band (peer maxDepth **43**). The ratio direction was an open measurement, not a prediction, and it moved the opposite way from S1b: **7 % above parity to 3 % below** | **measured, git 2.55.0 — no movement, as predicted.** ×1.01 (ours 53,308,805 B vs peer 53,025,451 B; 35,003/35,003 objects), unchanged from S1b's ×1.01 — blob and tree shapes are unchanged too (blob base=20,000 delta=0; tree base=460 delta=9,540 maxDepth=49): the hash term already fully orders every tie on this fixture, leaving the search bound nothing new to decide | **measured, git 2.55.0 — a gain, not the predicted small movement.** ×1.04 (ours 29,290,667 B vs peer 28,127,048 B; 15,711/15,711 objects — the branch's own history grew again since S1b's 15,677/15,677), down from S1b's ×1.09. Blob maxDepth stays saturated at the cap (**50**) on both tools, unlike `DELTA_CHAIN` — real, branchy history is not tie-dense enough for the depth-scaled bound to pull chains off the cap the way it does on the synthetic corpus. Blob base falls from 3,740 to 3,603; tree base rises from 969 to 1,131 |
| **S3** | promotion + max-depth non-admission (§4i, DC-8) | window residency | **measured, git 2.55.0 — confirms the prediction: exactly zero, byte-identical to S2 in every field.** ×0.97 (ours **181,396 B**, unchanged; peer 187,097 B; 900/900 objects). Blob base=7 delta=293, maxDepth **44** (peer 43) — every field matches S2. On a single straight chain the predecessor is already the most recent window member, so promotion has nothing to move, and max-depth non-admission never fires (maxDepth stops at 44, short of the 50 cap) | **measured, git 2.55.0 — the prediction was wrong: non-zero, and the pack got larger.** ×1.01 (rounds the same), ours **53,310,650 B** vs peer 53,025,451 B (unchanged; 35,003/35,003 objects, unchanged) — up **1,845 B** from S2's 53,308,805 B. Blobs stay shape-identical (base=20,000 delta=0). Trees move: base falls from 460 to **449** (−11), delta rises from 9,540 to **9,551** (+11), maxDepth falls from 49 to **48**. **Counterintuitive and worth flagging on its own: 11 more deltas and a chain one level shallower would normally cost fewer bytes, not more — here the pack grew anyway.** Max-depth non-admission is proven inert here **by elimination**, not inference: `deltify.ts`'s guard is `chainDepth >= policy.maxDepth`, `policy.maxDepth` is 50 at defaults, and `MEDIUM`'s tree maxDepth was already 49 at S2 — one below the threshold the guard fires at — so no object in this corpus could ever have reached the guard's condition, before or after this stage. The mover is promotion: `MEDIUM`'s trees are not one chain but a forest where the same base repeatedly serves several targets, and reordering which one is tried first (the just-used base, not FIFO) flipped 11 tie outcomes — exactly the histogram-shaped tell this row's own prediction named in advance | **measured, git 2.55.0 — moved, but not a clean stage signal.** ×1.08 (ours 30,249,963 B vs peer 28,136,006 B; 15,732/15,732 objects — the branch's own history grew again since S2's 15,711/15,711), up from S2's ×1.04. The object count moved between measurements — this corpus is the branch's own evolving history, not a frozen fixture — so the byte delta is confounded with real commits landing in between and is not attributable to this stage alone |
| **S4** | 50-byte floor — **ratified (ADR-834)** | which objects are offered | **measured, git 2.55.0 — the structural prediction was already true before this stage; the growth prediction did not occur.** ×0.97 (ours **181,396 B**, unchanged; peer 187,097 B; 900/900 objects) — byte-identical to S3 in every field: blob base=7 delta=293 maxDepth=44 (peer 43), tree base=300 delta=0. The predicted "roughly 300 × (a deflated tree − a deflated tree delta), a few KiB" growth did not happen, because there was nothing left for the floor to move: the tree readout was already base=300 delta=0 as far back as B0, and S2's search bound independently refuses every 40-byte tree on its own — a no-incumbent budget of `floor(40/2)-20=0` refuses outright, floor or no floor | **measured, git 2.55.0 — the predicted small growth, confirmed.** ×1.01 (rounds the same), ours **53,310,676 B** vs peer 53,025,451 B (unchanged; 35,003/35,003 objects, unchanged) — up **26 B** from S3's 53,310,650 B. Blobs stay shape-identical (base=20,000 delta=0). Trees move by exactly one: base rises from 449 to **450** (+1), delta falls from 9,551 to **9,550** (−1), maxDepth unchanged at **48** — one small object crossed out of delta candidacy, matching the predicted class though far smaller in absolute terms than the `DELTA_CHAIN` estimate implied | **measured, git 2.55.0 — moved, but not a clean stage signal, as every prior row on this corpus has been.** ×1.05 (ours 29,679,338 B vs peer 28,143,459 B; 15,749/15,749 objects — the branch's own history grew again since S3's 15,732/15,732), down from S3's ×1.08. The object count moved between measurements — this corpus is the branch's own evolving history, not a frozen fixture — so the byte delta is confounded with real commits landing in between and is not attributable to this stage alone |
...
ordering gain is S1b − B0; the bound is S2 − S1b and improved the ratio rather than costing
it, contrary to its original faithfulness-trade rationale; promotion is S3 − S2 and was
measured to be zero on `DELTA_CHAIN`'s single chain but non-zero on `MEDIUM`'s tree forest,
where the same base serves several targets and reordering which one is tried first moved 11
ties; the floor is S4 − S3, measured at zero on `DELTA_CHAIN` — the search bound already
excluded its 40-byte trees on its own — and a 26-byte gain on `MEDIUM`.* The published figure
is S4. Any later comparison against these numbers must name the row it compares.
