...
records; promotion is S3 − S2 and was measured to be zero on `DELTA_CHAIN`'s single chain but
non-zero on `MEDIUM`'s tree forest, where the same base serves several targets and reordering
which one is tried first moved 11 ties.* Any later comparison against these
