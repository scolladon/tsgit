...
- The comparator is gen-then-date **always** (DC-5 as designed; the shipped choice is (b), see
  the decision table's review note): required whenever `minGeneration > 0` (git
...
| DC-5 | merge-base heap comparator with a graph | (a) generation-then-date always (degenerates to today's date order without a graph); (b) replicate git's switch exactly (`minGeneration === 0 && !correctedDates` → date only), needing a graph-metadata probe on `readCommitMeta` | **(a)** as recommended; **(b) shipped** | The review's git probe on a `commitGraph.generationVersion=1` graph (`deep` at topological level 4 with date …103, `shallow` at level 1 with date …500) showed `git merge-base d e` answers `shallow` — the DATE order — where (a) would answer `deep`: the plain-`merge-base` early break makes traversal order result-bearing with a graph, so (a)'s premise ("results are order-independent") was false. The interop row pins it; ADR-840 records the refinement. |
