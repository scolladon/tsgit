...
| Warm memo read (`all()` and `offsetTable()` directly; `lookup()`'s internal read) | 1 extra promise per memo read (each went through an `async` wrapper) | 0 — the synchronous `get` returns the memoised promise itself (`lookup()` remains `async` and keeps its own result promise) |
