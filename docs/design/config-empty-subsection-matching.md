...
### Resolved (folded-scope ADR phase)

- **N1 — old-name matching model** — **resolved by ADR-324** (accepted): raw byte-exact matching via `rawSectionName`; `parseSectionName` and the family guard are deleted; the design above is written to this model.
- **N2 — newline/NUL bytes in the NEW subsection** — **resolved by ADR-325** (accepted): `rejectSubsection`'s `\n`/`\0` refusal stays as a documented divergence on git's self-corrupting foot-gun; the interop rename matrix excludes the LF row (refusal-side unit test instead).
- **N3 — exported `InText` primitive surface** — **resolved by ADR-326** (accepted): both primitives re-shape to `(text, oldName, …)` raw-name form; `reports/api.json` regenerates.
