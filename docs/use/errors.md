...
| `CONFIG_BAD_NUMERIC_VALUE` | `key, source, value, reason` | An int-typed config key is present-but-valueless (git NULL) at a command that reads it numerically. Today scopes `core.loosecompression` / `core.compression`. Refuses **eagerly** on the same broad operational surface as the `[core]` path-likes (`status`, `log`, `branch`, … die; `config --get` / `--list` still survive). Reconstructs git's **single-line** refusal: `fatal: bad numeric config value '<value>' for '<key>' in file <F>: <reason>`, where `reason` ∈ `{'invalid unit', 'out of range'}` (`value` is `''` for the valueless case). Structurally distinct from `CONFIG_MISSING_VALUE`: **one** line (no `error:` prefix), the file token is **unquoted**, and there is **no `at line <N>`** — hence a separate code with no `line` field. When a valueless string `[core]` key and a valueless int `[core]` key co-exist, the earlier-by-file-line key's shape is reported (git's per-entry order). |
