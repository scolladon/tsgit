...
| `CONFIG_MISSING_VALUE` | `key, source, line` | A string-typed config key git dies on is present-but-valueless (git NULL) at a command that reaches it. Covers the identity / remote families (`user.name`/`email`; `remote.<n>.url`/`pushurl` eagerly at both `fetch` and `push`, even when a usable URL resolves), `branch.<n>.merge`/`remote` (eagerly in `pull`, before any network, across every `[branch *]` section), `merge.<n>.driver`/`name` (lazily at any real 3-way content merge — `merge`/`cherry-pick`/`revert`/`rebase`/`stash`), and the `core` path-likes `excludesFile`/`attributesFile`/`hooksPath` (eagerly on every command except the porcelain `config`/`init`/`clone`). The first valueless key by config-file line is reported (key lower-cased, subsection verbatim). Reconstructs git's two-line refusal: `error: missing value for '<key>'` + `fatal: bad config variable '<key>' in file '<F>' at line <N>`. Distinct from the absent case (`AUTHOR_UNCONFIGURED` / `REMOTE_NOT_CONFIGURED` / feature-off fallback) and from an empty-string value (git does not die on `''`). Porcelain reads (`config --get` / `--list`) still succeed on valueless keys. |
