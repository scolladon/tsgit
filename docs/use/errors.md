...
| `DIRECTORY_NOT_EMPTY` | `path` | A directory delete on a non-empty target, or a `rename` of a directory onto a non-empty directory (node on every platform — including Windows, where the node adapter emulates POSIX's kind rules and the removal's own `ENOTEMPTY` maps to this code — and memory). On linux only, a non-directory renamed onto a destination that is one of its own ancestors reports this instead of `PERMISSION_DENIED`, because the kernel checks emptiness before non-directory-ness. Also raised by a ref write whose reflog path is occupied by a directory holding real logs: an all-empty directory tree there is removed and the write proceeds, anything else refuses with that path, reproducing git's "there are still logs under" rather than clobbering them. |
| `FILE_EXISTS` | `path` | An exclusive create (`writeExclusive`) against anything already occupying the path — a regular file, a directory (empty or not), or a symbolic link, including a dangling one; also the lock-file contention signal. A ref transaction reuses this code for git's name-availability check: creating `refs/heads/a` while `refs/heads/a/b` exists refuses here, `path` naming the blocking ref's loose path even when the blocker is packed or held in a reftable. |
| `FILE_NOT_FOUND` | `path` | Read against a path that does not exist. |
| `NOT_A_DIRECTORY` | `path` | Directory operation against a non-directory, or a regular file or symlink on an ancestor segment of a write's path (the reported path is adapter-dependent at the immediate parent: node `FILE_EXISTS`, memory `NOT_A_DIRECTORY` carrying the ancestor; higher up a rename destination's ancestor chain, node carries `dst` on POSIX and `src` on Windows, memory the blocking ancestor); also a directory renamed onto a regular file or a symlink (node on every platform, memory). A ref transaction reuses this code for the other half of git's name-availability check: creating `refs/heads/a/b` while `refs/heads/a` exists refuses here, with the blocking ref's loose path. |
| `OPERATION_ABORTED` | — | An `AbortSignal` fired during an operation. |
| `PERMISSION_DENIED` | `path` | Filesystem permission error, including lexical out-of-root rejections, **write**-side symlink-escape rejections (a symlinked leading directory or leaf resolving outside the root set), and 8.3 path mismatches on Windows. A **read** through a symlink resolving outside the root set is not refused — reads are lexical, matching git. Also a non-exclusive write (`write`, `writeUtf8`, `writeStream`, `appendUtf8`) over a directory or a symlink at the leaf, and a non-directory renamed onto a directory (node and memory anchor `src`; the browser adapter anchors `dst`) — except on linux, where a destination that is one of the source's own ancestors reports `DIRECTORY_NOT_EMPTY` instead, because POSIX does not order the two checks (memory still reports `PERMISSION_DENIED` there). |
| `RESOURCE_LOCKED` | `resource, path, mtimeMs?` | A `.lock` file already exists; another writer holds it. `path` is the lock file's own path; `resource` says which lock — `'index'` (`index.lock`, carrying `mtimeMs` when a stale-lock break window is configured), `'commit-graph'`, or `'ref'` for `packed-refs.lock`, which refuses **every** ref delete, since a delete cannot know whether it must rewrite `packed-refs` until it has looked. A contended `<ref>.lock` is `REF_LOCKED`, not this code. |
| `UNSUPPORTED_OPERATION` | `operation, reason` | Feature not available in this adapter / runtime. Also renaming a directory onto a destination inside itself, or the root into itself: `operation: 'filesystem'`, `reason` the invalid-argument errno name; carries no `path`. No longer raised by the ref store: deleting a packed ref (`delete-packed-ref`) and renaming a packed-only remote-tracking ref (`rename-packed-tracking-ref`) are both supported — [`updateRef`](primitives/update-ref.md) rewrites `packed-refs` without the entry, and [`remote.rename`](commands/remote.md) writes the new names loose. |
...
| `INVALID_OBJECT_HEADER` | `reason` | Loose object header could not be parsed. Also a loose **commit, tree or tag** whose header's size claim disagrees with the inflated body (`size mismatch: header says N, actual content is M`) — git buffers those three types and checks the length. A **blob** with the same defect is *not* refused here: git streams blobs, so [`readObject`](primitives/read-object.md), `readBlob`, [`catFile`](commands/cat-file.md), [`streamBlob`](primitives/stream-blob.md) and `checkout` all serve its real bytes. `verifyHash: true` refuses it with `OBJECT_HASH_MISMATCH` instead. |
| `INVALID_OBJECT_ID` | `value, reason` | String → `ObjectId` parsing failed. |
| `INVALID_PACK_ENTRY` | `offset, reason` | Packfile entry malformed. Also raised when a base entry's — or a delta's instruction stream's — inflated byte count differs from its own header-declared size, in either direction (`reason: 'bad object: inflated size differs from declared size'`) — git's uniform `unpack_entry_data` check, applied by [`readObject`](primitives/read-object.md) / `readBlob` / [`streamBlob`](primitives/stream-blob.md) and by pack ingestion (`fetch` / `clone`) alike. `offset` is the entry's own byte offset into the pack. |
...
| `OBJECT_HASH_MISMATCH` | `expected, actual` | Stored object bytes do not hash to the id they were read under. The hash covers the object **as stored**, loose header included — so an object whose header size claim lies refuses here even when its body is intact, whatever its type. Raised by `readObject` / `readBlob` / [`streamBlob`](primitives/stream-blob.md) under `verifyHash: true`, and by [`updateRef`](primitives/update-ref.md) verifying a ref's new target before the compare-and-swap — which puts it on every command that writes a ref through it, [`tag.create`](commands/tag.md) and [`clone`](commands/clone.md) included. `expected` is the id asked for, `actual` the id the stored bytes hash to. |
| `OBJECT_TOO_LARGE` | `id, actualSize, limit` | Object exceeds `maxBytes` cap. |
| `OBJECT_NOT_FOUND` | `id` | Id missing locally and (if applicable) the promisor remote did not deliver it. Also raised by [`updateRef`](primitives/update-ref.md) when a ref's new target is not present (git's ref transaction verifies before it swaps), by [`tag.create`](commands/tag.md) for a lightweight tag naming a missing object, and by [`clone`](commands/clone.md) when the advertisement's HEAD, a tag, or a remote-tracking ref names an object the fetch did not deliver. A delete, a null new id, and a symbolic write are never verified, so none of them can raise it. |
...
| `UNEXPECTED_OBJECT_TYPE` | `id, expected, actual` | Resolved object's type does not match the caller's expectation (e.g. asked for tree, got blob). Also raised by [`branch.create`](commands/branch.md) for a start point that does not peel to a commit — `expected: 'commit'`, `actual` the fully-peeled type, and `id` the resolved object's own oid (peeled through any annotated-tag chain — the mismatched tree/blob's id, never the tag object's). Git's `fatal: not a valid branch point: '<start>'` line is composed by the caller from the start point **as typed** — the library returns structured data, not rendered text (ADR-249, ADR-861). Also raised by [`updateRef`](primitives/update-ref.md) for a non-commit written to `HEAD` or `refs/heads/*` — `expected: 'commit'`, typed by the **name passed**, never by what a symbolic ref resolves to; every other ref namespace accepts any existing type, so a lightweight [`tag.create`](commands/tag.md) over a blob is fine — and by [`clone`](commands/clone.md) for an advertised HEAD that is not a commit. [`streamBlob`](primitives/stream-blob.md) raises it for a loose non-blob at the `await`, not at the first chunk. |
...
| `BRANCH_NOT_FULLY_MERGED` | `name` | An **unforced** `branch.delete(...)` against a branch whose tip is not reachable from its merge reference — git's `branch -d` safety valve, which `force: true` skips. The reference is the branch's configured upstream when that names a commit, and `HEAD` otherwise; an upstream *replaces* `HEAD` rather than widening the test, so a branch merged into its upstream but not into `HEAD` counts as merged, and one merged into `HEAD` but behind its upstream does not. Both sides are peeled to commits, so a tip or reference standing on an annotated tag is measured at the commit the tag names. A branch that is itself a symbolic ref is deleted unchecked, and an unborn `HEAD` with no upstream leaves nothing merged. |
...
| `REF_CHAIN_TOO_DEEP` | `depth, limit` | Symbolic ref chain exceeded the recursion cap. The cap counts the refs one read resolves, the terminal included — git's `SYMREF_MAXDEPTH` — so **four** symbolic hops resolve and a fifth refuses. A chain exactly five hops deep used to resolve and now refuses. |
| `REF_CYCLE_DETECTED` | `chain` | Symbolic ref pointed at itself directly or indirectly. `chain` holds the walked names in order, the repeated one last. Raised by ref resolution and by [`updateRef`](primitives/update-ref.md) walking a symbolic `name` before it writes — that walk has no depth cap of its own (git's does not either), so a cycle is what ends it; `REF_CHAIN_TOO_DEEP` belongs to the read-side resolvers. Also raised, with no symbolic ref involved at all, when one ref transaction's **delete run** names the same ref twice — git's `ref_update_reject_duplicates`, screened before any lock is taken; `chain` then carries that one name twice. |
| `REF_LOCKED` | `name` | Another writer holds `<ref>.lock`. The lock is taken before the ref's current value is read, so a delete of a ref that does **not** exist refuses too — git locks first and looks second. |
| `REF_NOT_FOUND` | `name` | Resolution against a missing name. **No longer** raised by an [`updateRef`](primitives/update-ref.md) delete: deleting an absent ref is a no-op, as `git update-ref -d` is. The porcelain still refuses an unknown name first, each with its own code — `branch.delete` `BRANCH_NOT_FOUND`, `tag.delete` `TAG_NOT_FOUND`, `remote.remove` `REMOTE_NOT_CONFIGURED`. |
| `REF_UPDATE_CONFLICT` | `name, expected, actual` | CAS check failed in [`updateRef`](primitives/update-ref.md). `expected` is compared against the value the **given** name resolves to once symbolic refs are walked — never an intermediate link's own content — while `name` is the name you passed. Commands that write through `HEAD` (`commit`, `merge`, `reset`, `cherryPick`, `revert` and their aborts) therefore report `name: 'HEAD'` when they lose a race. Either side may be `'absent'`. A wrong `expected` combined with an unusable new target reports the target refusal instead, because verification runs before the swap. |
...
| `INVALID_COMMIT` | `reason` | Commit object failed validation. Also raised by [`updateRef`](primitives/update-ref.md) — and so by every command that writes a ref through it, [`tag.create`](commands/tag.md) and [`clone`](commands/clone.md) included — for a new target that hashes correctly but fails git's own `parse_commit_buffer` acceptance, checked after the hash and before the compare-and-swap. Git's acceptance is transcribed as-is and nothing stricter is added, so a commit git tolerates (no author line, a tree id naming no tree) is tolerated here too. |
...
| `INVALID_TAG` | `reason` | Tag object failed validation. Same second source as `INVALID_COMMIT`: [`updateRef`](primitives/update-ref.md) refuses a new target that hashes correctly but fails git's own `parse_tag_buffer` acceptance, which puts it on [`tag.create`](commands/tag.md), [`clone`](commands/clone.md) and every other ref write. |
...
| `CONFIG_MISSING_VALUE` | `key, source, line` | A string-typed config key is present-but-valueless (git NULL) at a command that reads it for a real purpose. Covers identity (`user.name` / `user.email`), `remote.<n>.url` / `remote.<n>.pushurl`, `branch.<n>.remote` / `merge`, `merge.<d>.driver` / `name`, `submodule.<n>.url` / `update`, `extensions.objectFormat` (and, identically, `extensions.refStorage`), and the `[core]` path-likes `core.excludesFile` / `core.attributesFile` / `core.hooksPath`. Most keys refuse **lazily**, at the consuming command (`commit`, `fetch`, `push`, `pull`, `merge`, `submodule update`). The `[core]` path-likes refuse **eagerly**: `excludesFile` / `attributesFile` on every operational command (matching git's broad default-config death — `config --get` / `--list` still survive); `hooksPath` only when a command resolves the hooks dir (a documented narrower under-refusal — `log` / `diff` / `show` succeed where git dies). `extensions.objectFormat` refuses **eagerly, at `openRepository`** (Stage 2 of layout resolution), ahead of the acceptance gate. Reconstructs git's two-line refusal: `error: missing value for '<key>'` + `fatal: bad config line <N> in file <F>` (`extensions.objectFormat`) or `fatal: bad config variable '<key>' in file '<F>' at line <N>` (the other keys above — git's exact second line varies by call site even though the tsgit code is shared). Distinct from the absent case (`AUTHOR_UNCONFIGURED` / `REMOTE_NOT_CONFIGURED` / built-in defaults). Porcelain reads (`config --get` / `--list`) still succeed on valueless keys. Also covers `gc.reflogExpire`, `gc.reflogExpireUnreachable` and their `gc.<pattern>.*` forms, which [`reflog({ action: 'expire' })`](commands/reflog.md) validates in config-file order **before** it reads its own `expire` / `expireUnreachable` options, before it resolves a target, and before the repo-settings class — so a valueless entry refuses even when no ref matches its pattern and even when the run would have expired nothing. |
| `CONFIG_BAD_NUMERIC_VALUE` | `key, source, value, reason` | An int-typed config key holds a value git cannot parse as an integer — valueless (git NULL, `value` `''`), unparseable (`abc`, `1.5`, `5x`), or beyond the signed-64-bit range. Scopes `core.loosecompression` / `core.compression` / `core.packedGitWindowSize` / `core.packedGitLimit` (the streaming classes, validated **eagerly and fully** — git's `git_default_config` parity — on the same broad operational surface as the `[core]` path-likes: `status`, `log`, `branch`, … die; `config --get` / `--list` still survive) and `core.maxTreeDepth` / `core.deltaBaseCacheLimit` (the **repo-settings class**, git's `prepare_repo_settings` pair — validated at the object-store read and write, index, and commit-graph boundaries plus explicit per-builtin prologues, **not** the eager operational gate; see [`internals.md`](primitives/internals.md#assertrepossettingsvalid) and [ADR-859](../adr/859-repo-settings-are-validated-at-their-own-tier.md)). `core.deltaBaseCacheLimit` shares `core.maxTreeDepth`'s grammar — decimal / `0x` hex / leading-`0` octal, one optional `k`/`m`/`g` unit (× 1024ⁿ); `0` is accepted; a negative, unparseable, empty, or valueless value is `'invalid unit'`; a value beyond git's unsigned-64-bit range is `'out of range'` — and is skipped entirely (neither read nor validated) when the caller supplied an explicit `deltaBaseCacheMaxBytes` option (ADR-858). `core.packedGitWindowSize` / `core.packedGitLimit` share that identical grammar (`pack.windowMemory`'s own unsigned-long finder), but — being streaming-class keys, not repo-settings ones — are checked for the FIRST malformed entry of *either* key in file order rather than resolved last-wins, since git validates both in the same streaming callback; a well-formed value can only **lower** tsgit's pack window cache below its 64 KiB window / 16 MiB registry-wide defaults, never raise it (mmap reservations an eager heap-backed cache cannot honour as heap — [ADR-882](../adr/882-the-pack-window-cache-honours-gits-window-keys-as-upper-bounds-and-refuses-malformed-values.md)). Reconstructs git's **single-line** refusal: `fatal: bad numeric config value '<value>' for '<key>' in file <F>: <reason>`, where `reason` ∈ `{'invalid unit', 'out of range'}`. Structurally distinct from `CONFIG_MISSING_VALUE`: **one** line (no `error:` prefix), the file token is **unquoted**, and there is **no `at line <N>`** — hence a separate code with no `line` field. Both repo-settings keys share a validation model the streaming classes do not: each is validated on its **effective, last-wins** entry (an invalid line followed by a valid one succeeds), where the streaming classes die on *any* malformed line. Within its own tier the gate reports the first failing `[core]` entry by file line across the string path-likes and the streaming classes (git's per-entry order); the repo-settings class is a separate tier entirely, so — except for five commands (`status`, `commit`, `diff`, `bundle.create`, `rebase`) plus `maintenance` for `core.deltaBaseCacheLimit` alone, where git's own builtin reads the class ahead of its config pass — a malformed streaming-class entry on the same file is reported first, since the gate runs before any boundary touch. |
...
| `CONFIG_BAD_DATE_VALUE` | `value, key?, source?, line?` | A date-expression config value falls outside the accepted grammar: `never` / `false` (never expire), `now` / `all` (expire everything), `@<epoch>`, an ISO-8601 date, or a `<n>.<unit>.ago` relative expression. `value` is always present. `key`, `source` and `line` are present when the value came from a config entry the reader can name — `gc.reflogExpire`, `gc.reflogExpireUnreachable` and their `gc.<pattern>.*` forms, validated in file order by [`reflog({ action: 'expire' })`](commands/reflog.md) ahead of its options and its target — and absent when it came from a bare string with no config-file token to report (`gc.pruneExpire`'s already-defaulted value, resolved by `gc`). Reconstructs git's `fatal: bad date config value '<value>'`, with ` for '<key>' in file <F> at line <N>` appended when the location is known. |
