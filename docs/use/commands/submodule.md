...
a CLI `mode` wins without reading (or validating) the config value. A malformed
`submodule.<name>.active` value (git's boolean grammar) refuses
`CONFIG_BAD_BOOLEAN_VALUE`, checked once per call across every submodule
regardless of `mode`.
