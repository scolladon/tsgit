...
**Every advertised ref that gets written is verified against the objects the pack actually delivered**, the way git's own clone refuses to write a ref at an object it did not receive. A missing object, or bytes that do not hash to the advertised id, refuses `OBJECT_NOT_FOUND` / `OBJECT_HASH_MISMATCH` — for tags and remote-tracking refs as much as for the tracked branch. Types are checked only where git checks them: `refs/heads/<branch>` and a detached `HEAD` must name a **commit**, so a branch or a detached `HEAD` advertised at a tree or blob refuses `UNEXPECTED_OBJECT_TYPE { expected: 'commit' }`, while a tag or a `refs/remotes/origin/*` entry at any type is written. The symbolic `HEAD` a server advertises is not verified — a symref names a ref, not an object. Refs `clone` skips anyway (the literal `HEAD` entry, a name git's ref syntax refuses, namespaces outside branches, tags and remotes) are never verified either. **Any of these refusals rolls the whole destination back** — the gitDir is removed, so a refused clone leaves nothing behind to clean up ([ADR-864](../../adr/864-ref-updates-verify-their-target-as-gits-ref-transaction-does.md)).
...
- `OBJECT_NOT_FOUND` / `OBJECT_HASH_MISMATCH` — an advertised ref names an object the pack did not deliver, or one whose bytes do not hash to the advertised id. The destination is removed.
- `UNEXPECTED_OBJECT_TYPE` — the tracked branch or a detached `HEAD` is advertised at a non-commit; `expected` is `'commit'`. The destination is removed.
- `INVALID_COMMIT` / `INVALID_TAG` — an advertised ref names a commit or tag git's parser refuses. The destination is removed.
