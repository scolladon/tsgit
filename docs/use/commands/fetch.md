...
- **Prune scope.** Stale candidates are drawn from the merged loose-and-packed ref listing, so a **packed-only** tracking ref is pruned like any other — `packed-refs` is rewritten once for the whole run, in one ref transaction. A **symbolic** `refs/remotes/<remote>/HEAD` is never a candidate: git's prune scan skips symrefs outright rather than testing the branch name they happen to carry, so the symref and its target both survive a prune that removes every other stale ref. Local `refs/heads/*` and `refs/tags/*` are never in scope. Each pruned ref's coupled `logs/HEAD` entry carries `fetch: prune`.
...
- **No-op short-circuit:** when every wanted object already exists locally, `fetch` skips the pack round-trip entirely — no `fetch`/upload-pack exchange, no packfile written. The everything-local check rechecks the pack directory on a miss (`hasObject`'s `{ mode: 'recheck' }`), as git's own `check_exist_and_connected` does, so an object a concurrent lazy fetch or `git repack` already landed is not wrongly asked for again.
