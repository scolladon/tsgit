...
- `CONFIG_BAD_BOOLEAN_LITERAL` — `push.gpgSign` holds a value git's boolean grammar refuses; carries `{ key, source, value }`. Checked in the same eager pass as `INVALID_PUSH_DEFAULT`: when both `push.default` and `push.gpgSign` are malformed, the one on the lower config-file line is reported, regardless of which key it is.
- `REFSPEC_INVALID` — any configured `remote.<name>.fetch` / `remote.<name>.push` value fails git's own `parse_refspec`, **whatever remote you are pushing to**: git builds its whole remote table before the command runs any logic of its own, so one unusable value elsewhere in the config refuses the push. Carries `{ value, reason }`. The per-key grading is described on [`remote`](remote.md#throws).
