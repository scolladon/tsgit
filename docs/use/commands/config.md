...
- **Valueless keys (git NULL parity):** a `key` line with no `=` is a present-but-valueless entry — surfaced as `value: null`, distinct from `value: undefined` (key absent) and `''` (empty value after `key =`). Boolean interpretation maps `null` to `true` (`[core]` + `bare` ⇒ bare repository), and `getRegexp`'s `valuePattern` matches a `null` value as the empty string — both exactly like git (ADR-314). A malformed no-`=` line (`key ; c`, `bad!key`, `9key`) throws `CONFIG_PARSE_ERROR` where git dies `bad config line N`. Writes never emit valueless entries (git's CLI cannot either); `set` on a valueless entry replaces the line with canonical `key = value`, `unset` removes it (`previousValue: null`). Consuming commands that read a string-typed key for a real purpose — `commit` (`user.name`/`user.email`) and `fetch`/`push` (`remote.<n>.url`) — refuse a valueless entry with `CONFIG_MISSING_VALUE { key, source, line }`, reproducing git's two-line `missing value for '<key>'` / `bad config variable '<key>' in file '<F>' at line <N>` refusal. `--list`/`--get` still succeed on valueless keys, and `ParsedConfig` continues to treat valueless string fields as absent (ADR-315 D4). This resolves the deferred follow-up from ADR-315 (ADR-327/328/329; `docs/design/missing-value-refusal-parity.md`). An **int-typed** key reads differently: `core.loosecompression` / `core.compression` are validated **eagerly and fully** (git's `git_default_config` parity). A value git cannot parse as an integer — valueless, unparseable (`abc`, `1.5`), or beyond the signed-64-bit range — refuses `CONFIG_BAD_NUMERIC_VALUE { key, source, value, reason }`, reproducing git's **single-line** `fatal: bad numeric config value '<value>' for '<key>' in file <F>: <reason>` (one line, no `error:` prefix, unquoted file token, no `at line <N>` — a distinct shape from `CONFIG_MISSING_VALUE`). A value that parses but lies outside zlib's `-1..9` range (`10`, `99`, `-2`) refuses `CONFIG_BAD_ZLIB_LEVEL { level }`, reproducing git's bare `fatal: bad zlib compression level <N>`. Both refuse on the same eager-broad operational surface, reporting the first failing `[core]` entry by file line; `--list`/`--get` still succeed, and `ParsedConfig` treats an in-range int as `core.looseCompression` (honoured as the loose-object zlib level on write) and an invalid/valueless one as absent (ADR-353/354/355; `docs/design/int-config-valueless-refusal.md`). `core.maxTreeDepth` and `core.deltaBaseCacheLimit` are the other two int-typed keys read through `ParsedConfig`, and they share the same `CONFIG_BAD_NUMERIC_VALUE { key, source, value, reason }` shape, but neither sits on the eager-broad surface above: both are validated at a separate **repo-settings tier** — git's own `prepare_repo_settings` pair, reached at the object-store read and write, index, and commit-graph boundaries, plus explicit calls transcribing git's own per-builtin prologues, never the eager operational gate (see [`internals.md`](../primitives/internals.md#assertrepossettingsvalid) and [ADR-859](../../adr/859-repo-settings-are-validated-at-their-own-tier.md)). Their validation model still differs from the compression keys' the same way: git resolves each through a cached config-set lookup, so only the **effective, last-wins** entry is validated (an earlier invalid line overridden by a later valid one succeeds), whereas `core.compression` refuses on *any* malformed line regardless of what overrides it. `ParsedConfig` treats a valid value as `core.maxTreeDepth` (the tree-recursion depth cap, default 2048, honoured unclamped — see [`errors.md`](../errors.md)) or `core.deltaBaseCacheLimit` and an invalid one as absent — caught, for a command that reaches the repo-settings tier, at that tier rather than at open. **Scope divergence:** this porcelain's reads and writes span all four scopes (`system` → `global` → `local` → `worktree`, last-wins), but the library's own typed readers — including the one `core.maxTreeDepth` feeds — consult the **local** scope only, the same reader every other typed `core.*` key uses. A value set *only* in `~/.gitconfig` (global scope) is therefore honoured by canonical git and silently ignored by tsgit; `config --set --local core.maxTreeDepth <n>` is the reliable way to make it take effect.
- **Worktree scope:** gated on `extensions.worktreeConfig` in `local`, parsed with git's full boolean grammar (`true`/`TRUE`/`yes`/`on`/`1`/a valueless key all activate it — not only the literal lower-case `true` this used to require); inactive or absent throws `CONFIG_SCOPE_NOT_AVAILABLE`. A value git's grammar refuses throws `CONFIG_BAD_BOOLEAN_VALUE` instead — `extensions.worktreeConfig` is a discovery-tier key (alongside `core.bare`), so the malformed value refuses **every** command including this porcelain, before scope resolution is even reached.
- **Browser adapter:** `global` and `system` scopes throw `CONFIG_SCOPE_NOT_AVAILABLE` with `reason: 'browser-adapter'`.
- **Repository the acceptance gate rejects (`reason: 'repository-not-accepted'`):** `get`, `getAll`, `getRegexp` and `list` are the only commands on this porcelain that survive a repository the format-acceptance tier refuses — every write method still throws the carried format refusal. On the surviving four, the `local` and `worktree` scopes contribute nothing: a merged read (`scope` omitted) silently drops them and returns only what `global`/`system` hold, while naming `local` or `worktree` explicitly throws `CONFIG_SCOPE_NOT_AVAILABLE { scope, reason: 'repository-not-accepted' }` instead of reading the file. This is the same shared reason the ownership/trust gate raises for its own rejections — the literal names no particular gate on purpose.
...
- **Section-name case on READ (git parity):** section names resolve **case-insensitively** — `[CORE]`, `[Core]` and `[core]` are one section, and entries under all three merge with git's ordinary last-wins precedence. Subsection names stay **case-sensitive**: `[remote "Origin"]` and `[remote "origin"]` are two different remotes. This is the read path only; the raw old-name matching used by `removeSection` / `renameSection` (above) is byte-exact and deliberately unchanged, matching git's own asymmetry between resolving a key and naming a block to rewrite. Pinned against real git by a cross-tool interop test.
...
- **`push.default` and push-remote keys:** `[push] default` accepts one of `nothing`, `current`, `upstream`, `simple`, `matching` — case-sensitive (`Simple` is not accepted); `tracking` is a deprecated alias for `upstream`. Unset behaves as `simple` (applied by `push` at read time, not stored — absent stays absent). A present, non-valueless, unrecognized value is not silently dropped: `push` refuses eagerly — before resolving a remote or refspec, even given an explicit refspec — with `INVALID_PUSH_DEFAULT { value, source, line }`, reproducing git's `error: malformed value for push.default: <v>` / `error: must be one of nothing, matching, simple, upstream or current` / `fatal: bad config variable 'push.default' in file '<F>' at line <N>` refusal. `branch.<name>.pushRemote` is matched case-insensitively (`pushremote`); the subsectionless `remote.pushDefault` is read from a bare `[remote]` block with no subsection — a per-remote `[remote "x"] pushDefault` is not read. Both are lenient string fields like their siblings: a valueless entry is treated as absent, not refused. `--list`/`--get` return all three unchanged; see [`push`](push.md#behaviour) for how they combine into the resolved push remote and refspec (ADR-458).
...
- `CONFIG_SCOPE_NOT_AVAILABLE` — `reason: 'worktree-extension-unset'` (`worktree` scope without the extension), `reason: 'browser-adapter'` (`global`/`system` on the browser adapter), or `reason: 'repository-not-accepted'` (`local`/`worktree` on a repository the acceptance tier rejected).
- `CONFIG_BAD_BOOLEAN_VALUE` — `core.bare` or `extensions.worktreeConfig` holds a value git's boolean grammar refuses; carries `{ key, source, value }`. Both are discovery-tier keys (`assertRepository`), so the refusal fires on every command — this porcelain included — before any operation-specific config is read.
...
- `docs/adr/353-first-int-config-key-loose-compression.md` (first int-typed key `core.looseCompression`, parser, honoured at loose-object write)
- `docs/adr/354-int-typed-valueless-config-refusal.md` (`CONFIG_BAD_NUMERIC_VALUE` shape; eager-broad gate; cross-class file-line ordering)
- `docs/adr/355-eager-full-validation-of-core-compression-keys.md` (full validation: valued-invalid + `CONFIG_BAD_ZLIB_LEVEL`)
- `docs/design/int-config-valueless-refusal.md`
...
- `docs/adr/458-push-remote-and-push-default-canonical-git.md` (`push.default` enum + `tracking` alias, `branch.<name>.pushRemote`, subsectionless `remote.pushDefault`)
- `docs/design/rev-on-idx-write.md` (repo-wide strict boolean refusal: grammar, tiering, the twelve consuming keys)
- `docs/adr/627-boolean-config-values-are-refused-as-git-refuses-them.md` (`CONFIG_BAD_BOOLEAN_VALUE` / `CONFIG_BAD_BOOLEAN_LITERAL`; discovery/default-config/consumer tiering)
