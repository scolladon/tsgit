...
- **Uniform occupied-name refusals** — the node, memory and browser adapters agree on every write surface: an exclusive create (`writeExclusive`) refuses a file, directory or symlink occupant with `FILE_EXISTS`; a plain write refuses a directory or symlink leaf with `PERMISSION_DENIED`; `rename` refuses what POSIX `rename(2)` refuses. The memory adapter cannot be tricked into a state the node adapter would have refused. The agreement now reaches Windows too: the node adapter enforces the POSIX `rename(2)` kind rules and the exclusive-create symlink verdict itself there, since Windows' own filesystem calls do not.
