...
Honestly: the 1 ms budget bounds the bulk of the stall, but a residual stretch of roughly 7 ms of synchronous work sits between the budget's own checkpoints on this workload — `admit()` calls that would yield don't land often enough inside it to bring the tail down to the budget's own size. Pick `io: 'threadpool'` when the process cares more about event-loop latency than this workload's own throughput — other traffic sharing the loop, say. Pick the default when wall-clock time is what matters and nothing else shares the loop.
