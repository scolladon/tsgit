...
**Progress:** Phases 0–11 complete. `@scolladon/tsgit@1.0.0` published on npm with sigstore provenance (trusted-publisher OIDC). Phase 12.1 (clone smart-HTTP pack fetch), 12.2 (fetch + shallow + prune), 12.3 (push), 12.4 (clone bench), 13.1 (checkout materialize), and 13.2 (reset --mixed rebuilds the index) shipped. Remaining Phase 13.x items (`reset --hard`, 3-way merge tree walk) are next.
