...
- [ ] **23.2c** `status` faithfulness follow-ups (surfaced by 23.2b) — (1) promote git's type-change (`T`) and mode-only changes to first-class `ChangeKind` values across **both** columns (today both collapse to `modified` — `compareWorkingTreeEntry` on the working-tree side, ADR-254's projection on the staged side); a cross-column enrichment, larger than 23.2b. (2) "Unmerged paths" (stage 1/2/3) reporting on `StatusResult` (today `diffIndexAgainstTree` is stage-0-only, so conflicted paths are absent from the staged column).
