...
- [ ] **22.3b** Unify the two `subjectOf` "first line of a commit message" helpers (surfaced by 22.3a's architecture pass). `internal/history-rewrite.ts` uses `message.split('\n')[0]`; `internal/stash-message.ts` uses `message.indexOf('\n')` + `slice` — semantically identical but in two unrelated subsystems (history-rewrite commands vs `stash`), so 22.3a's bounded blast radius left it out. Resolve by extracting a single neutral `firstLine(message)` (likely a tiny `domain/` text util both can import) so the projection lives once; replace both call-sites and drop the per-module copies. Verify byte-faithfulness is unaffected (both already return the first line, empty-message → `''`).
- [ ] **22.4a** Interactive-rebase faithfulness + robustness follow-ups (surfaced by 22.4's mutation grind). (1) **Unrelated-history rebase divergence** — `rebaseRun` refuses a no-common-ancestor rebase with `UNSUPPORTED_OPERATION` ("no common ancestor between HEAD and upstream"), but canonical `git rebase <unrelated>` *succeeds* (empty merge-base → replays the whole branch, like `merge`'s `EMPTY_TREE_OID` add-add path). Pre-existing since 22.3 (#108), undocumented (no ADR) — a prime-directive divergence. Deliberately **not** pinned by a test during the grind (a test would cement the divergence). Resolve by either an ADR that documents + justifies the refusal, or making rebase faithful (replay against the empty-tree base). (2) **Empty reword/squash message rejected mid-replay** — an inline `reword`/`squash` message that cleans to empty currently throws `EMPTY_COMMIT_MESSAGE` from `stepReword`/`meldGroupMember` *after* HEAD has detached, leaving a partial, un-abortable rebase (no `rebase-merge/` state). Faithful + clean fix: reject empty messages **upfront** in `planInteractive` (mirroring the existing reword-without-message `INVALID_OPTION`), before any state change — leaving the replay's `allowEmpty: false` guards provably equivalent. Until then the two `BooleanLiteral` mutants there are knowingly-killable-but-deferred survivors.
