...
- [ ] **21.2a** git-faithfulness interop harness — extend the `cross-tool-interop` suite to assert write **porcelain** (`commit`, `mv`, …) against real `git`, not just primitives. Surfaced by 21.2: parity goldens are tsgit-computed (cross-adapter only), so a porcelain divergence from canonical git can ship undetected.
- [ ] **21.2b** `commit` — apply git message normalization (strip + single trailing newline, à la `git-stripspace`) so commit SHAs match canonical git. Today tsgit serialises the message verbatim, so any message without a trailing `\n` yields a non-faithful commit id (the `createCommit` primitive is faithful when handed a normalised message; the gap is the porcelain). Regenerates commit-id goldens across the suite.
