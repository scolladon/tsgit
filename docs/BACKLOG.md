...
- [ ] **19.5b** Browser-surface audit — namespace awareness. `tooling/audit-browser-surface.ts` matches only `readonly X: BindCtx<…>`, so the five nested-namespace CRUD commands (`config`, `remote`, `branch`, `tag`, `sparseCheckout`) fall off the browser-coverage gate. Teach the parser to capture `commands.*Namespace` bindings and the call-site scanner to detect dotted `repo.X.verb(` invocations, then bring those namespaces under the gate (adds a `config` parity/browser scenario or allowlist entry, since `config` has none today). doc-coverage already handles namespaces (20.8); this closes the matching browser-surface gap (ADR-194).
