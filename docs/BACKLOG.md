...
- [ ] **22.3a** Centralise the `readCommitData` / `treeOf` / `subjectOf` one-liners and `requireSymbolicHead` now triplicated across `cherry-pick.ts` / `revert.ts` / `rebase.ts` into a shared `history-rewrite/internal` helper (surfaced by 22.3's architecture pass; bounded blast radius kept it out of the rebase PR per YAGNI/KISS).
