...
- [x] **14.5** Phase 14.4 follow-up bundle — 12 of 14 sub-items shipped in one bundled PR; **14.5.3 abandoned** (see [Abandoned work](#abandoned-work)), **14.5.13 deferred**. Design: `docs/design/phase-14-5-followups.md`. ADRs: [049](adr/049-bundle-vs-split-followups.md) (bundle), [050](adr/050-cache-invalidation-policy.md) (cache invalidation), [051](adr/051-symlink-target-containment.md) (symlink target), [052](adr/052-directory-not-empty-error-code.md) (DIRECTORY_NOT_EMPTY), [053](adr/053-abandon-skip-resolve-optimization.md) (skip-resolve abandoned).
      ...
      - [ ] **14.5.13** Strip a leading `\\?\` extended-length prefix inside Windows `normalizeForCompare` (or document the limitation). Today device-namespace paths can spuriously deny in `lstat`/`creation` modes. (Security review LOW-4.) _Deferred — not abandoned. It was implemented in the §14.5 bundle, then reverted while bisecting the Windows-CI failure. The bisection cleared it: the 37-test failure was 14.5.3, not this strip. The strip is sound and can be reintroduced on its own; it was left out only to keep the bundle's Windows CI clean while 14.5.3 was diagnosed._
