...
- [ ] **23.3a** Consolidate the date-ordered **commit priority-queue** (`precedes` = commit-date desc, oid asc tie-break + sorted `enqueue`) into one shared domain helper. Surfaced by 23.3: the comparator now exists in three places — `domain/blame/priority-queue.ts` (generic, payload-carrying), `commands/describe.ts` (inline, payload-free), and `merge-base`'s equivalent date-ordered walk (the `precedes` doc-comment already cross-references it). Rule-of-three is met, but unifying touches `describe`/`merge-base` (outside blame's diff), so 23.3's architecture pass logged it here rather than widening that PR. Pick the generic `QueueEntry<T>` shape; re-mutation-test the three consumers after the move.
