...
- [ ] **23.4** API foundation & read model — an ergonomics pass laid **before** completing the Phase 23 inspection surface, because every remaining inspection command (23.5–23.8) builds on it. Surfaced by the 23.4 API review (capability-map + golden-path stress test + parameter audit): params are already cosmetic-free post-ADR-249, but the surface mirrors git's CLI 1-to-1 and the read Core is too weak to build on. Captures **every** finding from that review. Several sub-items change the public API, but breaking changes stay unconstrained until the end of Phase 28, so 23.4 sequences **purely by dependency** — no release-bundling. The read-model **convergence** (commands as thin projections over the model) is the capstone 23.4j, sequenced **last** and gated on the model proving out across the command set — respecting the over-design caution rather than forcing the abstraction early.
  - [ ] **23.4a** snapshot surface — **keep the name** `snapshot`: it is correct, not colliding. Git's own model is "snapshots, not deltas" (a commit/tree *is* a snapshot of state), and the type's iteration-stability invariant *is* database **snapshot isolation** — a consistent, lazily-materialised, point-in-time view isolated from concurrent writes — so `snapshot` is the precise term, and it lives on a power-tool surface, not the everyday porcelain. What stands (name-independent): stop exporting the `*Deps` / `create*Snapshot` wiring internals from `index.ts` (consumers use `repo.snapshot.*`, never hand-wired deps), and consider first-class source accessors (`repo.tree(rev)` / `repo.index` / `repo.workdir` / `repo.stash`) for ergonomics. **No rename** — only the leaked exports go. _(S7; E2's earlier "rename" reframed — the name is confirmed right.)_
  - [ ] **23.4b** History/commit view + folded subject — extend the read model with an **all-parents** (topo/date) commit walk and expose git's folded `%s` subject (`foldSubject`). Strengthens the too-weak **first-parent-only** `log` Core that today forces every history summary down to `walkCommits` + a hand-rolled subject fold. **Unblocks 23.5–23.8.** _(E1 + M2; folds in the `foldSubject` = git `%s` / `format_subject` port and the all-parents date-ordered walk.)_
  - [ ] **23.4c** `readFileAt(rev, path)` — golden-path convenience for the #1 viewer job "read a blob at a revision", today a 4-call primitive dance (resolve → readObject → readTree → walk → readBlob). Additive. _(M1.)_
  - [ ] **23.4d** merge/pull reshape — unify the interrupted-op state machine to a `merge.run/.continue/.abort` **namespace** (parity with rebase/cherryPick/revert, which are namespaces while merge is three flat methods); replace the `fastForwardOnly`/`noFastForward` boolean pair with `fastForward: 'only' | 'never' | 'allow'`; drop the internal-coupling `reflogLabel` from the public option. _(S1 + S2 + S5.)_
  - [ ] **23.4e** rev vocabulary — standardise the "which commit-ish" parameter on **`rev`** across blame/log/diff/merge/reset/describe/show/pull (five names today: `rev`/`from`/`target`/`input`/`branch`); reserve `from`/`to` for ranges only. _(S3.)_
  - [ ] **23.4f** infra/policy off op signatures — move `breakStaleLockMs` (add/mv/rm) and the clone SSRF policy (`resolver` / `allowInsecure` / `allowPrivateNetworks`) from per-call options onto context/adapter creation, where environment policy belongs. _(S4 + S6.)_
  - [ ] **23.4g** primitive Tier-2 audit — pare `.primitives.*` to a minimal, coherent extension set; review the low-level mutation leaks (`stageEntry` / `unstageEntry` / `setEntryFlags` / `recordRefUpdate` / `writeSymbolicRef` / `runHook`) — are they a real developer extension surface or git plumbing showing through? _(S8.)_
  - [ ] **23.4h** status↔diff correlation — `status` and `diff` return unrelated shapes, so a consumer stitches "staged vs unstaged vs the actual hunks" by hand. A richer `status` that links to its hunks; needs design. _(M3.)_
  - [ ] **23.4i** `blame` working-tree pseudo-commit — `blame` targets committed revs only; git's "Not Committed Yet" working-tree attribution is unimplemented (ADR-258). _(O2.)_
  - [ ] **23.4j** Read-model convergence (capstone) — once the command set has proven the model out, refactor the porcelain reads into thin projections over the read model. Sequenced **last** and gated on the over-design caution: force nothing until the right shape is evident from the full surface. _(E2's convergence half.)_
