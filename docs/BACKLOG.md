...
- [ ] **22.2c** abort/reflog audit follow-ups (surfaced by 22.2b). (1) **Detached `reset --hard HEAD` no-op** — `reset`'s detached path calls `recordRefUpdate(HEAD, …)` directly (not via `updateRef`), so the 22.2b no-op skip does not reach it; git skips the `HEAD` reflog on a detached no-move (a direct ref → needs-commit semantics) while tsgit writes a spurious `reset: moving to <oid>`. Add a caller-level gate + interop pin. (2) **`merge --abort` `HEAD` message** — `abort-merge` writes `merge: aborted`; real git writes `reset: moving to <oid>` on the `HEAD` symref. Align the message and pin it.
- [ ] **22.3** `rebase` (non-interactive). Lands the third `*-abort` consumer → extract the shared `abortSequencerReset` helper (22.2b item 2).
