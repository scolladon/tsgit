...
- [ ] **21.2c** `rm` — staged-change safety valve, faithful to `git rm`. `git rm <path>` refuses (exit 1, "use --cached to keep the file, or -f to force removal") when a path's staged content differs from `HEAD`; tsgit's `repo.rm` is unconditionally permissive (removes any index-matched path, no valve). Surfaced by 21.2a's `rm-interop`, which seeds via a commit to sidestep the valve. Add the refusal with `force`/`cached` overrides and a co-refusal interop case.
