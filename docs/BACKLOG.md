...
- [ ] **19.3a** AAA-marker semantic audit — pass-2 review of 19.3 found ~388 `it()` bodies where the autofix landed `// Arrange` directly followed by `// Assert` with no intermediate statement (single-statement tests). Markers satisfy the lint but carry no signal. Either (a) teach `detect-missing-aaa` to flag empty-section sandwiches, or (b) sweep the offenders and either delete redundant markers (single-statement bodies) or refactor to extract a `sut` variable.
- [ ] **19.3b** Scanner support for two-stage call shapes — `it.skipIf(cond)('title', body)` / `it.runIf(cond)('title', body)` are silently dropped by `scanItBlocks` (the title literal sits in the second `(…)`, not the first). Extend the scanner's `isEach`-style branch to cover `skipIf` / `runIf` so the surrounding heuristics fire on conditionally-skipped tests.
