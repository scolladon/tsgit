...
- [x] **14.4** Full Windows support.
      _Accepted:_ `NodeFileSystem` now reconciles 8.3 short-name vs long-name `realpath` outputs via a lazy `Promise<string>`-cached canonical root resolved at first call (`getCanonicalRoot`). Containment checks run through a new `pathContains(parent, child, isWindowsFn)` helper that case-folds on Windows and uses the platform-native separator. The constructor accepts an optional `isWindowsFn: PlatformPredicate` from the new `src/adapters/node/platform.ts` indirection — production uses `process.platform === 'win32'`; tests inject `() => true | false` to exercise both arms on a single host. `mapErrno` gained a first-class `ELOOP → PERMISSION_DENIED` arm; `openWithNoFollow` pre-lstats on Windows so `EACCES`/`EPERM` rewraps only when the leaf is actually a symlink (a real EACCES on a regular file surfaces unchanged). `runFs`-style unsafe `as NodeJS.ErrnoException` casts replaced with `isErrnoException` guards. The shared `test/unit/ports/file-system.contract.ts` now uses `nodePath.join` everywhere instead of template-string concatenation. `.github/workflows/ci.yml` re-adds `windows-latest` × `node 22, 24` to the `unit-tests` matrix; mutation / integration / E2E remain Linux-only (ADR-044). Real-runner Windows tests live in `test/unit/adapters/node/node-file-system-windows.test.ts` (skipIf POSIX) so they execute on the matrix cell. Design: `docs/design/phase-14-4-windows-support.md`. ADRs: [041](adr/041-windows-testing-strategy.md) (dev-vs-CI), [042](adr/042-canonical-root-lazy-realpath.md) (canonical-root cache), [043](adr/043-errno-mapping-placement.md) (errno placement), [044](adr/044-ci-matrix-windows-inclusion.md) (CI matrix), [045](adr/045-separator-normalisation-policy.md) (separator policy).
