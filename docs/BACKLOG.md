...
- [ ] **14.5** Phase 14.4 follow-up backlog — items surfaced by the three review passes performed on `chore/phase-14-4-followup` but deferred per the "no architectural changes in a follow-up PR" rule. Each is a tracked stand-alone refactor:
      - [ ] **14.5.1** Cache `normalizeForCompare(rootDir)` + `normalizeForCompare(canonicalRoot)` as instance fields on `NodeFileSystem`. Eliminates ~4-6 string allocations per `checkContainment` call on Windows; biggest win on `status` / `add --all` hot paths. (Perf review HIGH.)
      - [ ] **14.5.2** Bounded-concurrency walk in `NodeFileSystem.removeTree`. Replace the serial `for…of` over directory children with a `Promise.all` over batches of 8 entries. (Perf review HIGH; not §14.4-introduced — pre-existing.)
      - [ ] **14.5.3** Gate `policy.resolve()` in `checkContainment` on `policy.isAbsolute(abs)` to avoid a no-op `resolve` (which still touches `process.cwd()` + normalisation) on absolute inputs. (Perf review MEDIUM.)
      - [ ] **14.5.4** Parent-directory realpath LRU (capacity 64) in `resolveForCreation` to amortise the O(D) walk across writes into the same directory tree on a clone/checkout. (Perf review MEDIUM.)
      - [ ] **14.5.5** Replace `this.lstat(real)` inside `rmRecursive` with a direct `runFs(() => this.fsOps.lstat(real), path)` — removes a redundant `checkContainment` round-trip on an already-verified path. (TS review MEDIUM-4.)
      - [ ] **14.5.6** Mirror the read-mode pre-`realpath` containment check in `resolveForMode`'s `lstat` branch so obvious out-of-tree inputs fail-fast before any I/O. (TS review MEDIUM-3.)
      - [ ] **14.5.7** Narrow `makePolicy`'s parameter type from `typeof nodePath.posix` to an explicit structural interface matching the methods actually consumed, to prevent accidental injection of the host `nodePath` namespace. (TS review MEDIUM-5.)
      - [ ] **14.5.8** Move the `nativePolicy` default out of `src/repository/find-layout.ts` (which today imports across the hexagonal boundary — repository → adapter). Either inject the policy at the call site or hoist the default into a non-adapter module. (TS review LOW-7.)
      - [ ] **14.5.9** Validate absolute-symlink targets against rootDir containment inside `NodeFileSystem.symlink` (currently the link entry is contained but the target string is passed raw to the kernel, enabling an in-rootDir symlink to point outside it — an info-oracle via `readlink`). Relative targets pass unconditionally. (Security review MEDIUM, pre-existing behaviour; tighten now that the canonical-root machinery is in place.)
      - [ ] **14.5.10** Add `case 'EISDIR': return permissionDenied(path)` to `mapErrno` so Windows symlink-to-directory failures take a single hop instead of `EISDIR → UNSUPPORTED_OPERATION → isWindowsSymlinkRefusal → PERMISSION_DENIED`. (Security review LOW-1.)
      - [ ] **14.5.11** Drop the always-`true` `isSymlinkLeaf` argument at the only `isWindowsSymlinkRefusal` call site (or thread the real lstat result through). The current signature reads as if it carries information it does not. (Security review LOW-2.)
      - [ ] **14.5.12** Distinguish `ENOTEMPTY` from `ENOTDIR` in `mapErrno` (currently both map to `NOT_A_DIRECTORY`). Add a new error code or comment the deliberate coalescing. (Security review LOW-3.)
      - [ ] **14.5.13** Strip a leading `\\?\` extended-length prefix inside Windows `normalizeForCompare` (or document the limitation). Today device-namespace paths can spuriously deny in `lstat`/`creation` modes. (Security review LOW-4.)
      - [ ] **14.5.14** DI-level coverage of `openWithNoFollow(path, 'write')` against a Windows-mocked symlink — today the `'write'` flag-selection arm is only exercised via the POSIX-only locked-directory integration test. (Test review HIGH-2.)
