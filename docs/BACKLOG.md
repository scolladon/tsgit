...
- [ ] **22.2b** abort/sequencer follow-ups (surfaced by 22.2a). (1) **No-op reflog skip** — real git writes *no* reflog entry when `*-pick`/`revert --abort` resets to the **same** oid (lone conflict, branch never moved); tsgit's `updateRef` writes `reset: moving to <oid>` unconditionally, so both `cherryPick.abort` and `revert.abort` emit a spurious entry in the no-move case (the move case is faithful + interop-pinned). Make the abort reflog write conditional on a real ref move — touches both abort paths (and audits other `updateRef` callers for the same no-op). (2) **`abortSequencerReset` extraction** — `cherryPickAbort`/`revertAbort` are now byte-identical bar three injected points (operation label, `readXHead`, `clearXHead`); rule-of-three triggers once `rebase --abort` (22.3) lands the third consumer — extract a shared internal helper then. (3) **`cherryPickAbort` guard mutant** — a pre-existing `ConditionalExpression` survivor on `if (source === undefined && seqHead === undefined)` (`seqHead === undefined`→`true`); the distinguishing state (sequencer present, no `CHERRY_PICK_HEAD`) is unreachable for cherry-pick via the public API (unlike revert's markerless empty-stop), so it is likely provably equivalent — confirm + document `// equivalent-mutant`, or kill if a reachable state exists.
