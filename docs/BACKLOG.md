...
- [ ] **23.2a** Cosmetic-output sweep (breaking, v4) — enforce the "Structured output, not cosmetics" rule (CLAUDE.md / ADR-249, established during 23.2) across the **existing** command surface. Audit every command for options whose only job is to drive rendered text and for pre-rendered result fields, then **remove the option + its implementation code**, leaving the structured data. Known offenders: `show` (`bytes`/`text`, the full `--pretty`/`--format` engine, `--date=<mode>`, `--stat` widths, `-s`/`--no-patch` framing, `--abbrev` — ADRs 240/244/246/247/248 and the whole `domain/show/{date-*,pretty-*,diff-stat,...}` rendering subsystem); `log`/`reflog`/`diff`/`status`/`cat-file` re-audited for any display-only knob or rendered string. For each: keep the data fields, delete the renderer + option, and move the byte-faithful reconstruction into the interop test (the `describe` pattern). Group the removals into one major-version bump; supersedes the rendering ADRs it touches. New commands already comply, so this is debt cleanup on pre-rule surfaces only.
- [ ] **23.2b** `status` staged (index-vs-HEAD) column — `status` currently surfaces only the working-tree column (index-vs-worktree); `indexChanges` is never populated (the "staged" column is approximated, per the note in `status.ts`). Implement the real index-vs-HEAD diff so `status` reports git's "Changes to be committed", and `describe --dirty`/`--broken` (23.2) detect **staged-only** changes faithfully (`git diff-index HEAD` semantics over both columns). Surfaced by 23.2's `--dirty`, which inherits the gap and today reports clean for a staged-only change.
