...
- [ ] **23.1b** `show` v2 flags (deferred from 23.1, documented in `design/show-object-output.md` §1.1). Additive options on the existing `ShowResult`/`ShowOptions` shape — no breaking change: `-s` / `--no-patch` (suppress the commit patch), `--format` / `--pretty` (alternate commit headers), `--stat` / `--numstat` (diff-stat summaries), `<rev>:<path>` blob lookup, combined merge diffs (`-c` / `-m` / `--cc`), and alternate `--date=` modes. Each is pinned by extending the `show-interop` cross-tool parity suite.
