...
- [ ] **22.2a** `cherry-pick` faithfulness + test follow-ups (surfaced by 22.2). (1) Align `cherryPick.abort`'s reflog to git's faithful `reset: moving to <oid>` (it currently writes a non-faithful `cherry-pick: aborted`; `revert.abort` is already faithful per ADR-224) — behaviour-preserving except the reflog string, pinned by interop. (2) Close the mutation gap on `CHERRY_PICK_MERGE_NO_MAINLINE`'s display message in `domain/error.ts` (a surviving `StringLiteral` mutant — no test asserts the rendered message; mirror revert's `error.test.ts` message assertion).
