...
- [ ] **13.7** Defensive path validation at the index parser. `src/domain/git-index/index-parser.ts` admits IndexEntry paths containing `..`, `.`, empty segments, or leading `/`. Phase 13.6 added a defensive segment check at the synthesis boundary (`assertSafePath` in `src/application/primitives/synthesize-tree-from-index.ts`), but the canonical fix is to reject unsafe paths when the index is parsed — every downstream consumer (`materializeTree`, `checkout`, `reset`, etc.) then trusts the branded `FilePath` invariant.
      _Accept when:_ `parseIndex` (in `src/domain/git-index/index-parser.ts`) throws `INVALID_INDEX_ENTRY` for any entry whose path contains `..`, `.`, empty segments, or starts with `/`. Remove `assertSafePath` from `synthesize-tree-from-index.ts` once the parser is authoritative.
- [ ] **13.8** Bounded-size object reads for clean-merge content. Phase 13.4a's `buildContentMerger` calls `readBlob` for ours/theirs/base sequentially and unconditionally loads each into memory. Pass-1 perf review flagged it as a HIGH (parallelisation deferred so the security HIGH on memory pressure stays bounded). Need a `readObject({ maxBytes })` option that rejects oversized blobs upfront — pair with `mergeContent`'s `isBinary` short-circuit so multi-hundred-MB binary conflicts never materialise into memory.
      _Accept when:_ `readBlob(ctx, id, { maxBytes })` throws `OBJECT_TOO_LARGE` for blobs exceeding the cap; `buildContentMerger` reads the three blobs in `Promise.all` with `maxBytes: MAX_CONFLICT_OUTPUT_BYTES`; a property test confirms merging two adversarial 100 MiB blobs throws fast (under 200 ms) rather than OOMing.
- [ ] **13.4b** Three-way merge conflict handling. Phase 13.4a ships the clean-merge tree walk only. Conflicting merges currently throw `MERGE_HAS_CONFLICTS`; this follow-up should write per-path conflict markers to the working tree, write stage-1/2/3 index entries, and persist `.git/MERGE_HEAD` / `MERGE_MSG` / `ORIG_HEAD` so the user can resolve and commit.
      _Accept when:_ a conflicting merge leaves the working tree with `<<<<<<<` markers for resolved-merged-with-conflict paths, the index has stage-1/2/3 entries, and `repo.merge({ target })` followed by manual `add` + `commit` produces a clean merge commit.
