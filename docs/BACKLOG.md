...
- [x] **14.5** Phase 14.4 follow-up backlog — all 14 sub-items shipped in one bundled PR. Design: `docs/design/phase-14-5-followups.md`. ADRs: [049](adr/049-bundle-vs-split-followups.md) (bundle), [050](adr/050-cache-invalidation-policy.md) (cache invalidation), [051](adr/051-symlink-target-containment.md) (symlink target), [052](adr/052-directory-not-empty-error-code.md) (DIRECTORY_NOT_EMPTY).
      ...
      - [x] **14.5.3** Gate `policy.resolve()` in `checkContainment` on `policy.isAbsolute(abs)` to avoid a no-op `resolve` (which still touches `process.cwd()` + normalisation) on absolute inputs. (Perf review MEDIUM.)
