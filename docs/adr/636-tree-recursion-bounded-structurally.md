...
Two things this measurement does **not** license. It is not evidence that async recursion is stack-safe in general: the four sites rewritten above are `async` too and genuinely overflow at 2250/1350/925, with a real `RangeError` in the suite as proof. The difference — these six await genuinely-pending I/O at every level, and an awaited frame is suspended rather than stacked — is consistent with the result but is not fully characterised here. It also surfaced a second wall that was **not** a stack limit: `flattenLevel`, `diffChangedSubtree` and `walkLevel` rebuilt their cycle-detection ancestry with an O(depth²) `[...stack, id]` copy at every level, which exhausted the JS **heap** near depth 32000 — an uncatchable abort rather than a typed refusal, and reachable by configuration alone once the cap became user-supplied and unclamped. It was first recorded here as deferred; review escalated it, and it is **fixed at three of the four sites**: `walkTree`, `flattenLevel` and `walkLevel` now carry one walk-owned mutable `Set<ObjectId>`, added on entry and removed when the level returns. Live pointers there drop from O(depth²) to O(depth), so the 32000 figure no longer describes those three. Cycle semantics are unchanged — the set is exactly the root-to-current path, so two siblings sharing one subtree oid still both traverse, which is what the removal on exit guarantees.

**`diffChangedSubtree` could not take the shared `Set`, and takes an immutable cons list instead.** It is the one site whose sibling descents run *concurrently*: `diffRecursiveLevel` fans changed subtrees out through `boundedMap`, so several descents share one cursor at the same instant. A shared mutable set would let one sibling observe another's ancestry — a false `TREE_CYCLE_DETECTED`, or an entry removed while a concurrent sibling is still below it; that conversion was attempted during review and reverted when the hazard surfaced. The ancestry is therefore a cons list: each level allocates one node pointing at its parent, so siblings share their common ancestors structurally instead of copying them. Live pointers fall to O(depth) exactly as at the other three sites, the lookup stays the same O(depth) walk the array's `includes()` already cost, and immutability is what keeps concurrent siblings isolated. With that, the heap ceiling this record describes no longer applies to any of the four.
...
`merge.ts`'s `Promise.all` equivalent-mutant comment is carried into a different structure and its proof does not survive the move; it is re-proved against the new shape or removed. Every `ObjectId` cycle-detection stack changed. `walkTree`, `flattenLevel` and `walkLevel` replaced their per-level array with one walk-owned `Set`, which the explicit-stack shape made cheap; `diffChangedSubtree` took an immutable cons list instead, because its siblings descend concurrently (above). All four are now O(depth) in live pointers.
