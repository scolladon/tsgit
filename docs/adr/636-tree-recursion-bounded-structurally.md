...
**`diffChangedSubtree` deliberately keeps its per-level copy, and the reason is structural rather than an oversight.** It is the one site whose sibling descents run *concurrently*: `diffRecursiveLevel` fans changed subtrees out through `boundedMap`, so several descents share one cursor at the same instant. A shared mutable set would let one sibling observe another's ancestry — a false `TREE_CYCLE_DETECTED`, or an entry removed while a concurrent sibling is still below it. Concurrent siblings each need their own immutable snapshot of the path, and an O(depth) copy per level is what that costs. Converting it was attempted during review and reverted for exactly this reason. So the heap ceiling this record describes still applies to `diffTrees` with `recursive: true` at extreme configured depths; bounding it would mean changing how that walk fans out, which is a separate change.
...
`merge.ts`'s `Promise.all` equivalent-mutant comment is carried into a different structure and its proof does not survive the move; it is re-proved against the new shape or removed. The `ObjectId` cycle-detection stacks changed for three of the four sites — `walkTree`, `flattenLevel` and `walkLevel` replaced their per-level array with one walk-owned `Set`, which the explicit-stack shape made cheap. `diffChangedSubtree` keeps its array, because its siblings descend concurrently and each needs its own snapshot (above).
