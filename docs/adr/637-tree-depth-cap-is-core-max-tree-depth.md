...
- **An invalid value refuses repo-wide**, matching Pin 7: git parses `core.*` at startup and goes fatal on `write-tree`, `ls-files`, `cat-file -p`, `ls-tree` and `status` — commands with no depth surface at all. The refusal sits in the **eager operational gate** (`assertOperationalRepository`), not in `readConfig`, and the mechanism was arrived at by refuting the alternative rather than by preference. Two `readConfig`-side shapes were tried against the binary and both fail: a throw inside `assembleParsed` poisons the one cached entry that also carries `tokens`, killing the seven invalid-value finders and the `config` porcelain — which git keeps alive (`config --get`, `--list`, writes and `init` all exit 0 on a repo with an invalid value); and a strict per-entry applier refuses an invalid-then-valid sequence where git exits 0, because **git validates this key's effective last-wins value**, unlike `core.compression`, which dies on any malformed line. The gate is checked ahead of the lowest-line reduction: `maxTreeDepth` is reported first regardless of line order. This is the pattern ADR-355 already built for exactly this class, and the finder is modelled on `findFirstInvalidCompression`.

  **No new error code.** `CONFIG_BAD_NUMERIC_VALUE` already carries `{ key, source, value, reason }` with `reason: 'invalid unit' | 'out of range'` — the same union `parseGitInt` returns — and is already in `reports/api.json`, so this adds no public surface and gates only on the `docs/use/errors.md` row. The leniency documented at `applyLooseCompressionEntry` (*"valued-but-invalid int merges as absent"*) stands for every other numeric key; the asymmetry is deliberate, is documented at the strict site as a why, and generalising strictness to the rest of `core.*` is a config-subsystem decision this record does not make.
