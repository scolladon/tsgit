...
The seeded map must be tree-shaped: a key that would put a file where an earlier key already made a directory (the root included) throws `NOT_A_DIRECTORY` carrying the offending key, at construction time. Beyond seeding, the in-memory adapter mirrors POSIX refusals on every write surface, so a test that overwrites a directory or renames onto one fails there exactly as it would on Node.
